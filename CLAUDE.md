...
### 另一半:管道不许断头(PM 2026-08-01 拍板 · 与上条配套,不是豁免)⭐⭐⭐

> PM 原话:「自动化无法完成的,可以交给人工用最简单的方法完成。任务下发都要保持管道畅通,例如『今日待办』内,FDE 可以了解问题并且做出人工处理。」

上条治的是**偷懒甩手**(能自动化却丢给人)。这条治的是**烂尾**(自动化确实做不了,于是发现就死在日志里没人知道)。两条一起才完整:

**判断顺序(不许跳步)**
1. 能自动化 → 必须自动化(上条,深挖 UI / reverse API / 写进 ME 沉淀)
2. 确实做不了(按钮在第三方后台、需要真人决策、对方系统挡住我们)→ **不许停在这里**,必须下发成人工任务
3. 人工任务必须进**同一个管道**(今日待办 / 执行看板),不能只写进 cron summary、console.log、或某个只有开发看的表

**下发的人工任务必须自带三件套**(缺一条就是没下发好)
- **what** — 一句话说清问题**和影响**(「这个页面拿不到任何谷歌流量」,不是「index_verdict=NOT_INDEXED」)
- **how** — 具体到点哪里的做法,让 FDE 不用问人(「点页面上的『请求编入索引』,然后就不用管了」)
- **href** — 直达链接,连粘贴都不用(收录那条直接跳到谷歌**已定位到该网址**的检查页)

**反模式(绝不做)**
- ❌ 发现写进 `cron_run_logs.summary` / `console.error` 就算完事 —— 那不叫下发,叫埋掉
- ❌ 卡片只说「Resubmit for indexing」不说去哪点 —— FDE 得先研究一遍,等于没下发
- ❌ 系统做不了就默默跳过(silent skip)—— 静默降级必须同时产出一条人工任务或一条可见的数据断流标记

**真实事故(2026-08-01)**:CTS 一篇 blog 的 PR 开好后躺着没人知道(待办只统计 draft 不统计 pr_open);Oztop 2 个页面谷歌不收录、网站页面数据 65 天没更新(主机商挡了我们的服务器)—— 三件事系统全都「知道」,但没有任何一处会走到人眼前。修法见 `src/lib/pm-todo/manual-items.ts`(今日待办「🙋 需要你动手」栏)。

### 一句话(两条合起来)

> **能自动就别丢给人;真丢给人,就把话说到他不用问第二遍。发现不许死在日志里。**
