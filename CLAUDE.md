...
第三方 UI 卡住 / API 未文档化 / 跨客户重复操作 → **自己解决**：深挖 UI 隐藏入口 → 官方 Graph API 直调 → 页面 context inject Ajax → DevTools 深度自动化 → 沉淀成 ME 产品能力。

**绝对禁止**「请 PM/FDE/客户老板去点某个按钮」类人工 SOP。业务规模 = 自动化 × 无人工兜底。
