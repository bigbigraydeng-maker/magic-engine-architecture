 ...
 * own list, and this one sits next to it.
 *
 * Rule kinds:
...
export const A_RISK_RULES = [
  ...
  { prefix: 'supabase/migrations/', why: 'migration —— 数据库结构变更，不可逆' },
  { suffix: '.sql', why: 'SQL —— 直接改数据库结构或生产数据' },

  // --- The control plane, including this gate itself ----------------------
  { prefix: '.github/workflows/', why: 'CI 定义与本闸门自身' },
  { prefix: 'tools/ops-review-loop/', why: '决定 PR 自身命运的评级与复审控制面' },
  { prefix: 'tools/ai-orchestrator/', why: '编排控制面' },
  { exact: 'render.yaml', why: '生产服务与 cron 调度' },

  // --- Auth, permissions, tenant isolation --------------------------------
  { exact: 'src/middleware.ts', why: '请求级鉴权与路由边界' },
  { prefix: 'src/lib/auth/', why: '鉴权与客户访问边界' },
  { prefix: 'src/app/api/auth/', why: '鉴权端点' },
  { prefix: 'src/app/api/webhooks/', why: '外部入站 webhook —— 签名校验即安全边界' },
  { basenameWord: 'rls', why: 'RLS 行级隔离策略' },
  { basenameIncludes: 'isolation', why: '客户 / 租户隔离判定' },

  // --- Execution kernel: authorization, approval, idempotency, state ------
  { prefix: 'src/lib/kernel/', why: 'Kernel —— 执行授权、审批、幂等、状态机' },
  { prefix: 'src/app/api/kernel/', why: 'Kernel 端点' },
  { prefix: 'src/lib/execution/', why: '自动执行策略与背书 —— 决定系统自己会做什么' },

  // --- Money -------------------------------------------------------------
  { prefix: 'src/lib/billing/', why: '计费与用量' },
  { prefix: 'src/app/api/stripe/', why: '支付' },
  { prefix: 'src/app/api/mtc/', why: '客户额度扣减' },

  // --- Production scheduling ---------------------------------------------
  { prefix: 'src/lib/cron/', why: '生产定时任务注册与调度' },
  { prefix: 'src/app/api/cron/', why: '生产定时任务路由' },

  // --- Outward, hard-to-retract side effects ------------------------------
  { basenameIncludes: 'publisher', why: '对外发布 —— 发出去就撤不回来' },
  { prefix: 'src/lib/email/', why: '对外发信' },
  { basenameIncludes: 'guardrail', why: '花费 / 发布安全闸 —— 它松了别的全松' },
  { basenameIncludes: 'ssrf', why: 'SSRF 出网边界' },

  // --- Credentials --------------------------------------------------------
  { suffix: '.env', why: '凭证文件' },
  { includes: '/.env.', why: '凭证文件（带后缀变体）' },
  ...
  { basenameIncludes: 'cred', why: '凭证装载 / 额度' },

  // --- Supply chain (CLAUDE.md §4 counts a new dependency as a big task) ---
  { exact: 'package.json', why: '依赖清单 —— 引入新依赖即供应链面' },
  { exact: 'package-lock.json', why: '依赖锁 —— 引入新依赖即供应链面' },
]
 ...
 *
 * @param {string} path
 * @returns {{risk: 'A'|'B'|'C', why: string}}
 */
export function classifyFile(path) {
  const aRule = A_RISK_RULES.find((rule) => ruleMatches(rule, path))
  if (aRule) return { risk: 'A', why: aRule.why }

  const cRule = C_SAFE_RULES.find((rule) => ruleMatches(rule, path))
  if (cRule) return { risk: 'C', why: cRule.why }

  return { risk: 'B', why: '含实际业务逻辑，且不在严格低风险清单内' }
}
 ...
 * @returns {{risk: 'A'|'B'|'C', computedRisk: 'A'|'B'|'C', declaredRisk: string|null, readable: boolean, reasons: string[]}}
 */
export function classifyRisk({ files, declaredRisk = null } = {}) {
  ...
  for (const file of files) {
    ...
    if (typeof path !== 'string' || path === '') {
      readable = false
      computed = 'A'
      reasons.push('改动清单里有一项没有文件名 —— 输入不可信，按 A 处理')
      continue
    }
    const { risk, why } = classifyFile(path)
    computed = higherRisk(computed, risk)
    if (risk !== 'C') reasons.push(`${path} —— ${risk} 级（${why}）`)
  }
