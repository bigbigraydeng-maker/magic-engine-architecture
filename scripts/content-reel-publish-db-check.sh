...
#      (scripts/db-replay-and-verify.sh)
...
#   5. rollback script with data present (tables must be kept)
#
...
out=$(psql -v ON_ERROR_STOP=1 -X -q -At -d "$DB" \
  ...
  -c "SELECT to_regclass('public.content_reel_publish_attempts') IS NULL
           ...
           AND NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname LIKE 'content_reel_%' OR proname = 'set_content_reel_live_enabled')
...
echo "    PASS rollback (empty) removes tables, functions, trigger, columns"
...
VALUES ('a2000000-0000-0000-0000-000000000001', 'c2000000-0000-0000-0000-000000000001', 'CC', 'route_a', '{}', 'approved', 'https://x/cc.mp4');
SELECT public.content_reel_video_copy_start('b2000000-0000-0000-0000-000000000001','c2000000-0000-0000-0000-000000000001','a2000000-0000-0000-0000-000000000001','https://x/cc.mp4','content-factory/c2/authorized/cc.mp4','e1000000-0000-0000-0000-000000000001','agent');
SELECT public.content_reel_video_copy_finish('b2000000-0000-0000-0000-000000000001','ready','abababababababababababababababababababababababababababababababab',1000,NULL);
SQL

auth_sql() { # $1 attempt id, $2 form hash char
  printf "SELECT public.content_reel_authorize('%s','c2000000-0000-0000-0000-000000000001','a2000000-0000-0000-0000-000000000001','b2000000-0000-0000-0000-000000000001',NULL,'live','%s','ui_click','e1000000-0000-0000-0000-000000000001','staff@example.com','human','{}','abababababababababababababababababababababababababababababababab','1616575215312482')" "$1" "$(printf "$2%.0s" $(seq 1 64))"
}

TMP=$(mktemp -d)
Q -c "BEGIN" -c "$(auth_sql d2000000-0000-0000-0000-000000000001 1)" -c "SELECT pg_sleep(2)" -c "COMMIT" > "$TMP/a1" &
sleep 0.5
Q -c "$(auth_sql d2000000-0000-0000-0000-000000000002 2)" > "$TMP/b1"
wait
...
echo "    PASS concurrent authorise: one wins, the other gets $(grep -Eo '"code": "[a-z_]+"' "$TMP/b1")"
...
echo "==> 5/5 rollback with data present (tables must be kept)"
...
[ "$(Q -c "SELECT to_regclass('public.content_reel_publish_attempts') IS NOT NULL
             ...
             AND (SELECT count(*) FROM public.content_reel_publish_attempts) = 1")" = "t" ] \
