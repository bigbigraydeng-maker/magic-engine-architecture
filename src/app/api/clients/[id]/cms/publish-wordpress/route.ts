...
async function handleDraft(clientId: string, body: DraftRequestBody): Promise<NextResponse> {
  ...
  // Sanitize user content only; append GEO block AFTER sanitization so its
  // style="position:absolute;top:-9999px" and aria-hidden attributes survive.
  const content        = prepareCmsContent(rawHtml) + (geoSnapshot ? '\n' + geoSnapshot : '')
