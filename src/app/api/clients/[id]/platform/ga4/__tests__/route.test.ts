...
// contract, this time for google_ga4.
...
function makeGetRequest() {
  return new NextRequest(`http://localhost:3001/api/clients/${CLIENT_ID}/platform/ga4`)
}

function makeDeleteRequest(connectionId?: string) {
  const url = new URL(`http://localhost:3001/api/clients/${CLIENT_ID}/platform/ga4`)
...
describe('GET /api/clients/[id]/platform/ga4', () => {
  ...
  it('returns 200 with connections array on success', async () => {
    ...
    expect(body.connections[0].provider).toBe('google_ga4')
  })
})

describe('provider isolation — only google_ga4, 绝不碰别的平台', () => {
  ...
  it('returns empty when the client has no GA4 row, even with other providers present', async () => {
    mocks.listConnections.mockResolvedValue([GBP_SUMMARY])
    const body = await (await GET(makeGetRequest(), routeContext())).json()
    expect(body.connections).toEqual([])
  })

  it('DELETE refuses to remove a GBP connection through the GA4 route', async () => {
  ...
  it('DELETE succeeds for a genuine google_ga4 connection belonging to this client', async () => {
