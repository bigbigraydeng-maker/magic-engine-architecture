...
const mocks = vi.hoisted(() => ({ access: vi.fn(), allowed: vi.fn(), competitors: vi.fn(), collect: vi.fn() }))
...
describe('manual traffic direction route', () => {
  ...
  it('collects only active competitor domains and returns the audit receipt', async () => {
    ...
    expect(mocks.collect).toHaveBeenCalledWith(expect.objectContaining({ clientId: 'client-1', domains: ['wendywutours.co.nz'], maxChargeUsd: 0.15 }))
