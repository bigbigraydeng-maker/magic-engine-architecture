...
type ExternalSignal = { source_type: 'industry_news' | 'industry_media' | 'jobs'; source_name: string; source_url: string; title: string; excerpt: string; observed_at: string; valid_until: string | null; analysis?: { company_summary: string; job_summary: string; china_relevance: 'explicit' | 'related' | 'not_mentioned' | 'unclear'; relevance_reason: string; confidence: number } | null }
...
function ExternalSignalsSection({ signals, busy, message, onCollect }: { signals: ExternalSignal[]; busy: string; message: string; onCollect: (sourceId: string) => Promise<void> }) {
  ...
  return <section className="rounded-2xl border border-black/10 bg-white p-5" aria-label="行业动态和招聘信息">
    ...
    {latest.length ? <div className="mt-4 space-y-2">{latest.map(signal => <article key={`${signal.source_url}-${signal.observed_at}`} className="rounded-xl bg-me-ivory/60 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold text-me-charcoal/55">{labels[signal.source_type] ?? signal.source_name}</p><h3 className="mt-1 font-bold">{signal.title || signal.source_name}</h3></div><a className="shrink-0 text-xs font-bold underline" href={signal.source_url} target="_blank" rel="noreferrer">查看来源</a></div><p className="mt-2 line-clamp-3 whitespace-pre-line text-sm leading-6 text-me-charcoal/75">{signal.excerpt}</p>{signal.source_type === 'jobs' && signal.analysis && <div className="mt-3 rounded-lg border border-me-ochre/20 bg-white p-3 text-sm leading-6"><p><strong>公司信息：</strong>{signal.analysis.company_summary}</p><p><strong>岗位判断：</strong>{signal.analysis.job_summary}</p><p><strong>与 China Market 的关系：</strong>{signal.analysis.china_relevance === 'explicit' ? 'JD 明确提到 China / 中国' : signal.analysis.china_relevance === 'related' ? '与中国旅游相关，但没有直接点名' : signal.analysis.china_relevance === 'not_mentioned' ? 'JD 未提到 China / 中国' : '目前无法判断'}（{signal.analysis.relevance_reason}）</p></div>}<p className="mt-2 text-xs text-me-charcoal/50">观察于 {new Date(signal.observed_at).toLocaleDateString('zh-CN')} · 有效至 {signal.valid_until ? new Date(signal.valid_until).toLocaleDateString('zh-CN') : '未知'}</p></article>)}</div> : <p className="mt-4 rounded-xl bg-me-ivory p-4 text-sm text-me-charcoal/60">目前没有与客户目标市场相关的行业媒体或招聘信息。系统已自动过滤无关内容。</p>}
