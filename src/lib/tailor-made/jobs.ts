...
/**
 * 清掉这个 (client_id, itinerary_id, kind) 组合下卡住的僵尸任务。
 *
 * 必须在查 active / 插入之前做：数据库那道部分唯一索引（同一组合最多一条
 * queued/running）不知道"僵尸"这个概念，只要 status 还是 running 它就会
 * 挡住新任务插入。不先清，一条真正死掉的任务会把这个组合永久锁死。
 */
async function sweepStaleForKey(clientId: string, itineraryId: string, kind: TailorMadeJobKind): Promise<void> {
  await supabaseAdmin
    .from(TABLE)
...
/**
 * 建任务。同一份行程、同一种任务如果还有没跑完的，直接复用它而不是新开一条——
 * 防手抖连点两次上传，白白多花一次 AI 调用的钱。
 *
...
export async function createOrReuseJob(params: {
  ...
  const { clientId, itineraryId, kind, input } = params

  await sweepStaleForKey(clientId, itineraryId, kind)
  ...
  const { data, error } = await supabaseAdmin
    .from(TABLE)
    .insert({ client_id: clientId, itinerary_id: itineraryId, kind, input, status: 'queued' })
    .select('id')
    .single()
...
export async function markRunning(jobId: string): Promise<void> {
  await supabaseAdmin.from(TABLE).update({ status: 'running' }).eq('id', jobId)
}
