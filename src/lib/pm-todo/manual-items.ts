...
export function buildNotIndexedItems(
  ...
  for (const [clientId, agg] of Array.from(byClient.entries())) {
    ...
    items.push({
      ...
      how: `打开这份站内清单（已只筛未收录），每页都标了原因和该做的动作：「内容太薄」的，把正文补到 ${THIN_WORD_COUNT_THRESHOLD} 词以上、加内链；「爬过没收录」的字数已够，别再堆字，加内链后去 Search Console 点「请求编入索引」；「谷歌还不认识」的，去 Search Console 在最上方搜索框粘上这个网址、点「请求编入索引」。一次弄不完就先挑最想被搜到的几页`,
