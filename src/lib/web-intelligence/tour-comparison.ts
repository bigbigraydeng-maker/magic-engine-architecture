...
import { z } from 'zod'
import type { TourRecord } from './profiles/travel'
...
export type TourComparisonInput = {
  client_product: { name: string; source_url?: string | null; record: Partial<TourRecord> }
  competitor_product: { name: string; source_url: string; observed_at: string | null; record: Partial<TourRecord> }
  market_scope: string[]
}
...
function fieldValue(record: Partial<TourRecord>): Record<string, unknown> {
  return {
    name: record.name ?? null, duration_days: record.durationDays ?? null, price: record.price ?? null,
    promotion: record.promotion ?? null, route: record.route ?? null, departure_window: record.departureWindow ?? null,
