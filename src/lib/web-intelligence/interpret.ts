...
export const PROMPT_VERSION = 'web-signals-v5'
...
export function interpretationPrompt(signal: Signal, evidence: Evidence[], context: string, industryGuidance = ''): string {
  ...
  const payload = JSON.stringify({ context: context.slice(0, 2000), industry_guidance: industryGuidance.slice(0, 2000), monitored_scope: 'one configured business page', evidence_coverage: change.partial ? 'partial: additional changed text omitted, avoid broad conclusions' : 'complete changed lines', before: pick(before, change.before), after: pick(after, change.after) })
...
export async function interpretChange(signal: Signal, evidence: Evidence[], context: string, industryGuidance = '') {
  return callClaudeChat({ systemPrompt: SYSTEM, messages: [{ role: 'user', content: interpretationPrompt(signal, evidence, context, industryGuidance) }], maxOutputTokens: 1000, singleAttempt: true })
}
