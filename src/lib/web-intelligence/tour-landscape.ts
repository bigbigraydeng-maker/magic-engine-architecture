...
export function tourLandscapePrompt(input: { client_name: string; market_scope?: string[]; client_products: unknown[]; competitor_products: unknown[]; external_signals?: TourLandscapeExternalSignal[]; memory_context?: string }): string {
...
不要把不同旅行社的 Tour 强行一一对应，不要建议自动调价、发布或执行外部动作。`
  const context = `当前总览：${JSON.stringify(input.landscape)}\n\n${tourLandscapePrompt({ client_name: input.client_name, market_scope: input.market_scope, client_products: input.client_products, competitor_products: input.competitor_products, external_signals: input.external_signals, memory_context: input.memory_context })}`
  try {
