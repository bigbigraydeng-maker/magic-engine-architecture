 ...
 * 一条条钉住，不能跟网络调用绑在一起。
 *
 * 这个文件里**没有一行依赖 3CX 怎么给数据** —— 上游把话机记录翻译成
 * 下面这个 `CallRecord` 就行。电话这条线还卡在对方（要开分机、要配推送），
 * 但「电话进来之后系统怎么处理」不该跟着一起停在原地。
 *
 * **2026-08-05 这个分法当场兑现了一次。** 原计划的取数层是调 3CX 的 REST 接口，
 * 结果对方明确答复那些 endpoint 官方不提供、没文档、不保证长期可用，不建议
 * 用在生产环境；改成他们每 15 分钟往我们提供的数据库里推。整个取数层作废重来，
 * 而这个文件**一行没改** —— 它从来只认 `CallRecord`，不认数据从哪来。
 *
 * ─────────────────────────────────────────────────────────────────────────
 * 六条判断，和它们各自在防什么
 * ─────────────────────────────────────────────────────────────────────────
