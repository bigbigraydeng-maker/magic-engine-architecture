 ...
 *
 * 🔴 **durable execution**：`step.run(id, fn)` 把 authorize → measure → settle 装进同一 step，
 *    Inngest 重试整条 run 时，已完成 step 的返回值会**直接命中缓存**，不重跑。这层配合账本层
 *    的 `reservationId` 幂等（Codex P1/P2）与 provider 侧的幂等键（3b 接线时必须做），构成
 *    crash-after-provider-before-settle 场景的完整防线。
 */
...
export function parseRemeasureDue(raw: unknown): ParsedDue {
  ...
  if (typeof d.query_set_version !== 'string' || d.query_set_version.trim().length === 0) return { ok: false, reason: 'invalid_query_set_version' }
  if (typeof d.reservation_id !== 'string' || d.reservation_id.trim().length === 0) return { ok: false, reason: 'invalid_reservation_id' }
...
export function createGeoRemeasureOneFunction(deps: {
  ...
  return inngest.createFunction(
    ...
    async ({ event, step }) => {
      ...
      // 🔴 step.run 让 authorize→measure→settle 三步作为**一个 durable 单元**：
      //    Inngest 重试整条 run 时，已完成 step 的返回值直接命中缓存不重跑。
      //    step id 用 reservationId，确保重放击中同一 step。
