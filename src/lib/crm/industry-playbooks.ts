...
export const LOGISTICS_PLAYBOOK: IndustryPlaybook = {
  /**
   * 点了链接之后，还算「在看这件事」的窗口 = 14 天。
   *
   * 通用兜底是 30 天，旅游是 60 天（跟团游客人常提前几个月看）。货代反过来：
   * 询价到订舱是**交易型**的，节奏以天计 —— 2026-09-02 实测的一条真实对话里，
   * 客户从第一次问运费到追问空运截单时间只隔了两天。沿用 30/60 天窗口，
   * 一条早就找了别家的死线索会在名单上继续显示成「刚点过链接、还热着」，
   * 把真正在等报价的人挤下去。
   *
   * 14 天不是测出来的最优值，是「比一个货代询价的自然生命周期略长一点」的
   * 保守取值 —— 等这个客户跑满一轮真实数据后应该回来校准，而不是当成定论。
   */
  clickWindowMs: 14 * 86_400_000,
}
