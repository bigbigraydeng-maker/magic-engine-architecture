 ...
 *    任何一段代码 import 了这个文件，都不该因此拿到扣款能力。
 *
 *    注意别把这句话读成"那些动作接上 Kernel 就能做"：
 *    `lib/kernel/outward-authorization.ts` 里 v1 **无条件拒绝** `reversible !== true`
 *    的对外动作，而域名注册按定义就是撤不回来的。也就是说它今天在 ME 里
 *    **没有**合规落点——要么等 Kernel 契约单独评估后放开不可逆对外动作，
 *    要么做成"只读查询 + 人工下单"。这是 Phase 2 的硬前置门，不是实现细节。
 *
