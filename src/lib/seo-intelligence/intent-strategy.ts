...
export function buildBrandTrafficSplit(
  ...
  for (const keyword of keywords) {
    const bucket = isBrandedKeyword(keyword.keyword, brandRoot) ? totals.branded : totals.non_branded
    bucket.keywords += 1
...
export function prioritizeContentKeywords(
  ...
  return keywords
    .map(keyword => ({
      ...keyword,
      is_branded: isBrandedKeyword(keyword.keyword, brandRoot),
...
export function sortByIntentPriority<T extends IntentKeyword>(
  ...
  return [...keywords].sort((a, b) => {
    const aBranded = isBrandedKeyword(a.keyword, brandRoot)
    const bBranded = isBrandedKeyword(b.keyword, brandRoot)
