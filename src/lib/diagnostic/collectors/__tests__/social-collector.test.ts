...
const { mockScrapeInstagramProfile, mockScrapeFacebookPage } = vi.hoisted(() => ({
...
const DEFAULT_INSTAGRAM: Parameters<typeof mockScrapeInstagramProfile>[0] extends string
  ? never
  : {
      username: string
...
describe('SocialCollector.collect() — cache', () => {
  ...
  it('returns a valid CollectorResult from cache', async () => {
    const supabase = makeSupabase({ hasCachedFindings: true })
    const result = await new SocialCollector(supabase).collect(CLIENT_ID, DOMAIN, KEYWORDS)
