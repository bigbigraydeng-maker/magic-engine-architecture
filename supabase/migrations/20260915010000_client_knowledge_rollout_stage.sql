...
-- ────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.consume_knowledge_rollout_advance_request(
  p_request_id      UUID,
...
DECLARE
  ...
  v_event_id  UUID;
BEGIN
  -- 单次使用：只有 pending 状态才能被 claim。并发下两个提交同时跑到这里，
  -- Postgres 的行锁保证只有一个能拿到非空返回行。
