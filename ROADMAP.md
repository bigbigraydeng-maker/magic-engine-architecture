...
- [x] **P29.SEO.21** favicon cache-bust URLs - version the favicon link across public pages so Cloudflare/browser caches stop serving the old tab icon.
...
- [x] **P29.ADS.2** optional ads landing page - if paid-media demand justifies it, add a dedicated `/ads` landing page later; keep the first launch loop light for now.

### Website GEO Visibility - 2026-06-01
...
- [x] **P35.6** outreach email 生成 + 人审队列（2026-07-06，板桥×魏征双审后修复 merge）：segment 角度短调用生成 {subject,body}、合规 footer 代码固定（真实身份 + 收到原因 + 回复 no thanks 退订）、审核队列「左证据右邮件」一键复制标记已联系。**板桥 P0**：新增 `opted_out` 永久拒收终态（migration `20260706000002` **⚠️ 待 PM apply**——不 apply 拒收按钮会报错，第一封真实邮件发出前必须 apply）+ 队列/管线双拒收入口 + discover 去重天然拦截重导入。**待 PM**：Render 配 `OUTREACH_SENDER_NAME`（真人署名，UI 有黄条提醒）
- [x] **P35.7-infra** 生产 debug + 发现源切 Google Places（2026-07-06）：discover 生产报 DOCTYPE→502→(诊断埋点 cron_run_logs)→**DataForSEO business_listings HTTP 402 余额不足**。改造①discover 同步→异步 fire-and-forget（避 Cloudflare 100s 网关超时）②`res.json()` 全链路防御（暴露真实状态码）③**发现源从 DataForSEO 切到 Google Places Text Search**（`lib/places/business-discovery.ts`，用现有 `GOOGLE_PLACES_API_KEY`，绕开 402）④审计 OnPage 已是 `.catch(()=>null)` 降级，DataForSEO 全线不通时管线仍跑（少几个网站技术信号，机会分偏低，阈值待充值后回调）
- [ ] **P35.7** 转化闭环接线：replied → 发 `/discover` magic link；converted → `converted_client_id` 关联 clients
- **⚠️ 运维警报（2026-07-06）**：DataForSEO 账号 402 余额耗尽，影响 ME **所有** DataForSEO 功能（关键词排名 cron / SERP / OnPage / 外链 / 本地搜索 / 张骞客户发现）。PM 需查 app.dataforseo.com 充值。Prospecting 已用 Places 绕开，但现有客户数据采集可能静默失败。
- **战略收敛（PM 拍板 2026-07-06）**：砍餐厅/咖啡/按摩等内容/视觉型行业（批量生成内容不够精致，不适合靠视觉吃饭的品牌）。**聚焦 18 个高客单行业**，核心卖点 = 客资线索 + SEO + GEO。四柱打分卡后续把「社媒」柱降权，让 SEO/GEO/GBP/线索追踪当主角。
...
- [x] **P21.K.1 数据脊柱** ✅(1.5-2 天):Render Cron 每日拉取(复用 `getMetaTokenForClient` env 路径)+ 日度增量 + **额外 last_7d 拉 7d frequency**(不聚合日度)+ learning/delivery 探测 + `ad_daily_insights` 表(**migration 待 PM `go apply`**)+ 多客户隔离降级。补掉「无每日 campaign 级自动拉取」缺口
- [x] **P21.K.2 大脑落库** ✅(2-2.5 天):触发线引擎(config 驱动 + 冷启动 `insufficient_history` + 持续≠单日纪律)+ 漏斗层视图(per-client layers)+ 多视角对抗守门(证伪单日噪音/投放饥饿/learning 期噪音)+ budget_policy **execute 层硬闸** + 落 `ad_health_narratives`(**migration 待 PM `go apply`**)
- [x] **P21.K.3 仪表盘** ✅(2-2.5 天):`/dashboard/clients/[id]/ads-health` 分层视图(PM 默认 + 专业下钻,不平铺六区块)+ verdict 卡含**近 7 天原始值序列**(信任核对)+ **驳回按钮**(误报回灌 memory)+ 趋势自然语言注解 + 漏斗生意语言
...
| `22.E.S9` | **Action 卡片"预期影响"字段** — 90 天 traffic uplift 估算，规则可算无 AI | **P1** | ✅ PR #355 |
| `22.E.S4ext` | **SEO 列头客户事实快照** — 月点击 / 总曝光 / Page-1 词数（S4 扩展）| **P1** | ✅ PR #349 |
...
> **2026-06-04 ROADMAP 恢复说明**：PR #353（fix goals 入口）merge 时意外删除 PR #348/#350 的 S9-S13 + 决战日登记。本 PR 从 git 历史（commit e0d8d04）重新恢复，并把 A2/A3/A2-S9 的实际完成状态同步更新。同时加入 CLAUDE.md 黑线（任何 PR 动 ROADMAP.md，魏征必扫 diff 是否意外删别 Phase 的登记），防止再发生。
...
- **M5（FDE 价值闭环）**：S9（预期影响）+ S4ext（客户快照）上线，FDE 看 action 卡片立刻知道"做这个能拿到什么" ✅ 2026-06-04 上线
...
**S9 — Action 卡片"预期影响"字段（P1 优先）** ✅ 已上线 PR #355
...
**实施**（已完成）：
- 纯函数 `src/lib/seo-patrol/expected-impact.ts`（28 单测，魏征复审通过）
- 在 `StudioArticleTab.tsx` 顶部嵌入预期影响卡片（仅对有 keyword metadata 的 action 显示）
- 38 条策略性 action 静默不显示（不是降级，是正确——它们本就不是 keyword 工作）

**S4ext — SEO 列头客户事实快照（P1 优先）** ✅ 已上线 PR #349
...
实施（已完成）：复用 metrics endpoint 补 `gsc_avg_position` + 新建 `page-1-count` 轻量 API + `SeoColumnSnapshot.tsx` 组件。
...
| **A2 (S9)** | Action 卡片"预期影响"字段 | ✅ PR #355 merged |
| **S9-prereq** | 给 marketing_plan/diagnostic 也塞 action metadata | ⏭ 路线 A 跳过（38 条策略性 action 本就不该有影响估算） |
| **S13** | SEO 子类型精细分流（落地页 + 页面 SEO 优化）| 🔨 6/5-6/7 集中做 |
