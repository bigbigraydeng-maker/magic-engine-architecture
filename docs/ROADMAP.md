...
设计定稿：`~/.claude/plans/ads-impact-loop-capability.md`（§14 为验收条款）。阶段 0：PR #1656（看板查错列）、#1657（STATE.md 内核事实）、#1658（替代 #1080 的广告写路径归属校验 + 主页绑定只许员工改）。阶段 1：#1659（设置快照表 + 视频/广告组日数据）→ #1662（角色判定 + 结果阶梯配置界面）→ #1664（诊断 D1/D3/D4/D5/D7/D8）→ #1665（内部版日报）；#1660（行业词下沉剧本 + 回读去客户名）。

- [ ] **PM go**：按依赖顺序合并（#1659 → #1662 → #1664 → #1665），并单独授权 apply migration `20260914000001_ad_entity_snapshots_and_video_metrics.sql`、`20260914000002_ad_strategy_configs_outcome_ladder.sql`。表建好前新诊断不启用，原日报照旧。
...
- [ ] **AD-GADS-1 Google 广告数据接口拿到能用的权限，功能待实测 + 客户授权流程待确认**（2026-09-15）：旧申请（2026-06-11 提交）卡了三个多月，Google 那边旧审核流程已停用；2026-09-15 走新流程（品牌资料 + 域名验证 + logo 补齐）后自动拿到 **Explorer 级权限**——能管正式客户账户，2880 次操作/天，但**账户创建 / 用户权限管理 / 关键词规划 / 账单相关接口不能用**；**Basic 级**（不限功能、15000 次/天）待品牌审核通过，见日历提醒 2026-09-22 复查。
  - 🔴 **前置条件（必须先做，否则下面"待测"的结果没法解读）**：`src/lib/google-ads/client.ts:19` 和 `keyword-planner.ts:20` 把 `GOOGLE_ADS_API_BASE` 硬编码成 **`v17`，而这个版本 Google 已经停用**。拿停用版本去实测 Explorer 权限或 MCC 配置，收到的失败根本分不清是"权限不够"还是"版本被下线"。且 `listCampaigns`（`client.ts:137-153`）对 HTTP 错误的处理是 `console.error` 后 `return []`，不会往上抛错——这意味着 `/api/clients/[id]/google-ads/campaigns` 就算因为版本停用整个调用失败，也可能照样 200 返回一个空数组，实测者会把"版本下线"误读成"权限不够"或"这个客户没有 campaign"。**实测前先把两处 API 版本升级到 Google 当前支持的版本，并且确认 `listCampaigns` 对一个已知有 campaign 的账户返回的确实是非空结果**（而不是满足于"没报错"），再往下测权限/MCC。
  - **待测**：前置条件做完后，`src/lib/google-ads/` 现有封装（`client.ts` / `creds-loader.ts`）和 `/api/clients/[id]/google-ads/{campaigns,execute}` 这两条路由，拿一个已配 `google_ads_customer_id` 的测试客户跑一遍，确认 `GOOGLE_ADS_MANAGER_ID` 这个 MCC 账号本身是否已建、能否用 Explorer 权限正常调通（目前 `syncGoogleAds` 因权限未就绪一直 `success:false` 静默失败，见 `AD-ATTR-1`）。⚠️ 这一步测的是"调不调得通"，**不经过、也不能证明** `creds-loader.ts` 已把 `clients.google_ads_customer_id` 真正绑定生效——见下一条 IDOR。
  - 🔴 **`execute` 路由的实测只能用 Google Ads 的测试账户 + 专用测试 campaign，不能拿正式客户账户跑**：`/api/clients/[id]/google-ads/execute`（`execute/route.ts:162-235`）没有 dry-run，调用即刻真实生效——暂停/启用 campaign、改预算、加否定关键词，直接改客户投放和花费。如果"测试客户"其实是配置齐全的真实客户账户，跑一遍 `execute` 就是在动人家正在投的广告。要么专门开一个 Google Ads 测试账户（Google 提供 test manager account 机制）配一条不投放的测试 campaign 去跑 `execute`，要么这一步只验证只读的 `campaigns` 调用，`execute` 的权限验证改成查 Google Ads API 的权限/scope，不要真的调用会产生副作用的 action。
  - 🔴 **正式客户接入前必须先堵住 `execute` / `campaigns` 的账户归属 IDOR，否则共享 MCC 一接通多个正式客户就是"已调通"当成了"安全可用"**：`execute/route.ts:70-83,116-120` 只用 URL 里的 `clientId` 做 `requirePaidClientAccess`，随后**直接信任请求体里的 `google_ads_customer_id`** 去调用 Google Ads（`:116-117` 自己标了 `TODO P18.D: ... to prevent IDOR`，问题是已知的，没堵）；`campaigns/route.ts:37` 同样是从**查询参数**取 `google_ads_customer_id`，同一个洞。后果：A 客户的授权用户只要在请求里换成 MCC 能访问的 B 客户账户 ID，就能读 B 的 campaign，还能通过 `execute` 暂停/启用/改预算——**跨客户写权限**，不是理论风险。上面「待测」按现状描述的路径走一遍，也**验证不了这个洞已修**（它压根不检查传入的 `google_ads_customer_id` 是否等于该 `clientId` 在 `clients.google_ads_customer_id` 里配的值）。修：两条路由都要改成**只信任按 `clientId` 从 `clients.google_ads_customer_id` 解析出来的账户 ID**，忽略或校验请求体/查询参数里的值与之一致；这条必须列为**正式客户启用前置条件**，且要通过修好后的受约束路径重新验收，不能拿现状的「待测」结果背书。
  - ⚠️ **`keyword-planner.ts` / `/api/clients/[id]/google-ads/keyword-ideas` 不在这轮"正常调通"的测试范围内** —— 它调的 `generateKeywordIdeas` 正是上面标注的"关键词规划"接口，Explorer 权限下**预期直接被拒**。这条要等 Basic 级审核通过（见 2026-09-22 复查）后才该测"能不能调通"；在那之前跑出来的失败是权限限制，不是配置故障，别误判成 bug 去修。
  - ⚠️ **客户授权机制目前是两套互相冲突的说法，还没统一**：`src/lib/google-ads/client.ts` 支持**管理账户（MCC，`GOOGLE_ADS_MANAGER_ID`）** 统一代管所有客户账户的模式——客户在自己的 Google 广告账户里把 ME 的 MCC 加成关联管理账户（或 ME 从 MCC 发邀请、客户点接受），之后 FDE 只需把客户账户编号填进 `clients.google_ads_customer_id`（`/api/clients/[id]/google-ads-customer-id`），后端就能用公司统一权限操作。但这**不是代码目前唯一按此设计**——`GOOGLE_ADS_MANAGER_ID` 在 `client.ts` 里明确标注 `(optional)`，且现有两个面向客户的入口都没有走 MCC 关联：`src/app/authorisation/page.tsx:530-547` 让客户在 **Users 标签页直接邀请 `hello@magicengine.cloud` 为 Standard 用户**；`docs/sops/client-onboarding-access-requirements.md:40` 给 FDE 月付客户的话术同样是"Add user → Standard"，不是"关联 MCC"。这两套流程二选一都能跑通授权，但**不能同时当成"当前机制"**，FDE 会不知道该让客户走哪一步。**待确认（PM/技术需要拍板，不是自动统一）**：ME 的 MCC 账号是否已经真实建好；如果要切到 MCC 关联模式，`authorisation/page.tsx` 和 SOP 两处客户话术都要同步改掉，且要有迁移路径处理已经用 "邀请 Standard 用户" 方式授权过的老客户；如果继续用 "邀请 Standard 用户"，这条 AD-GADS-1 里描述 MCC 是"代码已按此设计"的说法要撤回，避免下一个 agent 照着这条去接 `GOOGLE_ADS_MANAGER_ID` 而忽略已经在跑的邀请流程。
  - ⚠️ 一旦真的开始写 `google_ads_pullback` 数据，会直接触发 `AD-ATTR-1` 那条"隐患已武装"——Oztop 已经同时配了 Meta + Google 两个账户，实测前先看 `AD-ATTR-1` 是否已修。
