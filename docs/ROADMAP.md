...
## 广告支柱 IMPACT 闭环升级 · 阶段 0/1 已上线，阶段 2（内核执行）开发中（2026-09-15）

设计定稿：`~/.claude/plans/ads-impact-loop-capability.md`（§14 为验收条款）。**阶段 0+1 全部 9 个改动已于 2026-09-14 由 PM 合并上线**：#1656/#1657/#1658/#1659/#1660/#1662/#1664/#1665/#1666，两张新表（`ad_entity_snapshots`/`ad_strategy_configs` 结果阶梯列）已建到生产。阶段 2（内核注册广告动作 + 预算锁 + 挪预算原子执行，按 §4.1/§14.1 K1–K14）开发中，均未合并、不涉及任何真实 Meta 写入：
- PR #1730（内核注册 4 个广告动作，子牙+魏征均已通过）
- PR #1732（AD-ADV-1，按打法显式传 Advantage+ 受众参数，子牙 CONDITIONAL PASS 已按要求更正、魏征通过）
- PR #1734（预算锁配置层，缺行/读错误一律锁定，复审进行中）

- [ ] **AD-ADV-1 后续（P2，PR #1732 子牙复审登记）**：`draft-and-gate.ts` 的 `createDraftForApproval()` 即时回读路径可以安全补"冷启动广告组 advantage_audience 被意外关闭"的反方向检查（不会像每日扫描路径那样误报存量广告组）——直接复用 `DRAFT_AUDIENCE_MODE[draft.kind]` 作为期望值传给 `checkLaunch`，不要用创建时间或名字启发式。按 §11 打分：频率低（无实测事故证据）/IMPACT 中/收入中 → P2。
- [ ] **PM 待拍板（阶段 2 前置）**：CTS 是否仍需「预算锁定」（决定 `budget_policy.locked` 的 CTS 初始值，见 PR #1734；默认所有客户统一锁定，不锁的客户需 PM 显式解锁）。
