...
- [ ] **AD-GADS-1 Google 广告数据接口拿到能用的权限，功能待实测 + 客户授权流程待确认**（2026-09-15）：旧申请（2026-06-11 提交）卡了三个多月，Google 那边旧审核流程已停用；2026-09-15 走新流程（品牌资料 + 域名验证 + logo 补齐）后自动拿到 **Explorer 级权限**——能管正式客户账户，2880 次操作/天，但**账户创建 / 用户权限管理 / 关键词规划 / 账单相关接口不能用**；**Basic 级**（不限功能、15000 次/天）待品牌审核通过，见日历提醒 2026-09-22 复查。
  - **待测**：`src/lib/google-ads/` 现有封装（`client.ts` / `keyword-planner.ts` / `creds-loader.ts`）和 `/api/clients/[id]/google-ads/{campaigns,execute,keyword-ideas}` 这几条路由，拿一个已配 `google_ads_customer_id` 的测试客户跑一遍，确认 `GOOGLE_ADS_MANAGER_ID` 这个 MCC 账号本身是否已建、能否用 Explorer 权限正常调通（目前 `syncGoogleAds` 因权限未就绪一直 `success:false` 静默失败，见 `AD-ATTR-1`）。
  - **客户授权机制**（代码里已按这个设计，行业标准做法）：ME 用**管理账户（MCC，`GOOGLE_ADS_MANAGER_ID`）** 统一代管所有客户，不是每个客户单独走一次 OAuth。客户要授权 ME 管理，需要**客户自己**在自己的 Google 广告账户里，把 ME 的 MCC 加成关联管理账户（或 ME 从 MCC 发邀请、客户点接受）——这一步谁都替不了客户点。客户点完之后，FDE 只需把客户的账户编号填进 `clients.google_ads_customer_id`（`/api/clients/[id]/google-ads-customer-id`），后端就能用公司统一权限操作该账户。**待确认**：ME 的 MCC 账号是否已经真实建好、邀请/关联客户账户的具体操作步骤要不要写成给 FDE 的 SOP。
