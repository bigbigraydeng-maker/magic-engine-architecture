...
- [ ] **AD-GATE-1 `approveDraft` 激活前不重新回读**：只查 `payload.status` 就 `activatePublished`，不重跑 `fetchAdSetReadback` / `checkLaunch`。草案在共用账户里躺几天，期间被改则批准人看到的是旧快照、钱按新配置花 —— 这违背 `launch-readback.ts` 自己"只有回读能看见"的立论。修：激活前重跑回读 + 闸门，有 blocker 拒绝激活。**不需 migration**
  ...
  - 🔴 **草案排期在等待批准期间会自然过期，跟"排期被篡改"不是同一类问题**（PR #972 最终轮 Codex review 指出，[讨论](https://github.com/bigbigraydeng-maker/magic-engine/pull/972#discussion_r3786595369)，Issue #981 closeout 补登）：`ad-publisher.ts:186-197` 建 `PAUSED` 草案时把 `start_time` 写成**创建那一刻**、`end_time = start + durationDays`；而 `approveDraft`（`draft-and-gate.ts:198-202`）调 `activatePublished`（`ad-publisher.ts:252-261`）**只把 campaign / ad set / ads 的状态改成 `ACTIVE`，从不重算或回读排期**。批准如果拖了几天，`start_time` 已经过去、实际投放期被悄悄压缩；要是批准发生在 `end_time` 之后，投放窗口直接归零，广告激活也可能因排期已过而跑不起来。⚠️ **这条跟本条正文"重跑回读比对排期"解决的不是同一件事** —— 比对只能抓住"有人把排期改了"，抓不住"排期本身随时间自然变旧"，就算没人碰过它也一样过期，**光校验"排期没被篡改"解决不了"排期已经过时"**。
    修：批准时**按批准发生的那一刻重新计算并回写 `start_time`/`end_time`**（保持原定投放时长不变），或者对已过期的草案直接判定失效、要求作废重建。**首投前置**，随 `AD-GATE-1` 一并处理，不需要单独排期
...
- [ ] **AD-FACT-1 事实来源从不校验，却盖"官网可溯"章**：`assertFacts` 只查 `sourceUrl` 非空，从不抓页面核对价格/地址/战绩，而 `traceClaims` 把原样传入的字段标成"官网可溯"。**第一条付费广告就会带着未核实内容投出去**，不是量大了才危险。⚠️ **修法不能是"按 `sourceUrl` 抓页核对"**（第二十四轮更正，Codex P1，核实成立）：**事实和 `sourceUrl` 是同一个调用方给的** —— 他完全可以指向一个自己控制、写着假价格假战绩的页面，抓下来照样"对得上"，然后拿到"官网可溯"的章。**拿请求体里的 URL 当信任根，等于没校验。** 而且直抓任意调用方给的 URL 还会引入 SSRF。
  ...
  5. 🔴 **`claims` 的字段来源本身会错配 —— 房源存在时，中介事实可能被错误标成"房源页可溯"，`bedrooms` 更是压根进不了可溯清单**（PR #972 最终轮 Codex review 指出，[讨论](https://github.com/bigbigraydeng-maker/magic-engine/pull/972#discussion_r3786595373)，Issue #981 closeout 补登）：`listing-draft-builder.ts:265-294` 的 `traceClaims` 把房源事实和中介事实**摊平进同一个 `factValues` 数组**（`:269-276`：`listing.address/suburb/priceLabel/status` + `agent.achievements/serviceArea`），命中后统一用 `sources.listing?.sourceUrl ?? sources.agent.sourceUrl` 标源（`:287`）—— **不管这句话实际来自哪个事实字段**。房源存在时，中介的战绩（`achievements`）、服务区域（`serviceArea`）如果命中，标注的却是房源页的 `sourceUrl`，不是中介自己的来源。而且 `ListingFacts.bedrooms`（`:29`）**没有进 `factValues`**，所以带卧室数的句子会被判成"固定话术 · 不含事实主张"，逐句可溯这道红线对它形同虚设。
     修：`factValues` 改成**逐字段**记录来源（哪个值对应哪个 `sourceUrl`），不是摊平数组配一个统一 fallback；把 `bedrooms` 补进可溯事实清单。**这一点没做完之前，不能只把当前 `claims` 原样搬到审批页展示给批准人** —— 那是把错误来源当"官网可溯"章盖给批准人看，后果跟"没做来源校验"一样。并入本条第 3 点（`claims` 持久化到审批页）的前置条件：**先修 provenance，再谈持久化/展示**
