...
- [ ] **AD-GADS-1 Google 广告数据接口拿到能用的权限，功能待实测 + 客户授权流程待确认**（2026-09-15）：旧申请（2026-06-11 提交）卡了三个多月，Google 那边旧审核流程已停用；2026-09-15 走新流程（品牌资料 + 域名验证 + logo 补齐）后自动拿到 **Explorer 级权限**——能管正式客户账户，2880 次操作/天，但**账户创建 / 用户权限管理 / 关键词规划 / 账单相关接口不能用**；**Basic 级**（不限功能、15000 次/天）待品牌审核通过，见日历提醒 2026-09-22 复查。
  ...
  - **待测**：前置条件做完后，`src/lib/google-ads/` 现有封装（`client.ts` / `creds-loader.ts`）和 `/api/clients/[id]/google-ads/{campaigns,execute}` 这两条路由，拿一个已配 `google_ads_customer_id` 的测试客户跑一遍，确认 `GOOGLE_ADS_MANAGER_ID` 这个 MCC 账号本身是否已建、能否用 Explorer 权限正常调通（目前 `syncGoogleAds` 因权限未就绪一直 `success:false` 静默失败，见 `AD-ATTR-1`）。
