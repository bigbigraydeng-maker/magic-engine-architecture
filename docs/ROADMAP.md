...
（#1637/#1640，见上），CTS 私信客服真正跑起来还需要 Inngest 编排把**已接收的 Messenger 事件**
消费掉、门户 UI、dry-run 验证等剩余步骤（见本节下方"剩余 issue"）。**范围已拍板只启动
Messenger AI 客服，WhatsApp 当前止于 webhook 接收和落库，不进入这条 AI 应答编排。**
...
- [ ] 剩余 issue（#1638/#1639 已合并，不再是剩余项，见上；Messenger/WhatsApp webhook**接收**
      ...
      Messenger 事件**的下游流程、门户 UI、dry-run 验证、Delivery day 灰度切换）——**范围
      已拍板只启动 Messenger AI 客服，WhatsApp 仅保留收件入库能力，不接入这条 AI 应答编排**
      ——**客户知识库这个前置依赖已经全部做完**（见下一节，6 步全部合并），这些是 CTS
      Messenger+WhatsApp v3 自己剩下的、不属于客户知识库范围的收尾项
