...
- [ ] **M2.7m 候选人轮转改成靠一列时间戳，别再靠小时数滚窗口** —— 现在挑人是
      「按 id 排序 + 按当前小时数滚动窗口」。能保证一天之内整份名单都扫得到，
      但填不上的人（模型一直答 unclear）每天照旧被重问一次，白花钱。
      正解是加一列 `stage_inferred_at`，试过就记一笔，下次排到最后。
      **加列是 A 级改动，要 PM 显式点头才能动**
