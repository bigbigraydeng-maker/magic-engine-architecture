...
- [ ] **M2.7b 卡片上直接记一笔 + 自动排下一步（界面那一半）** —— 后端已通（`parseNote`
      拿得到通话时间和时区，`segmentContact` 规则 3 到点自动捞回名单），缺的是卡片上
      那个输入框。现在要点开右侧抽屉、点四下 —— 销售一手拿电话时做不了。
      板桥原话：「这一个功能对我一天的影响，比三段式改版大得多。」PM 2026-08-15 拍板要做
- [ ] **M2.7c 三段式重排** —— 现在是 3 层 + 6 组 + 底栏 + 终端/同行开关 ≈ 10 个概念，
      CTS 员工要先学术语才能干活。改成「今天要联系的 / 今天联系过的 / 先放着」三段，
      分类词降级成卡片上一行「他为什么在这」。设计与三人评审意见见 PR #985 讨论
- [ ] **M2.7d 页面内回私信后立刻变灰** —— `messenger/send.ts` 发送成功不写
      `contact_touchpoints`（只写 `conversation_messages` + 审计），出站触点要等
      每小时同步才刷新 → 卡片最长一小时不变灰，正是 PM 抱怨的毛病换了个渠道
