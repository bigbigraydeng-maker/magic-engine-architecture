...
- [ ] **P35.14 冷邮件报告页(`/report/[id]`)升级为张骞完整版 Discovery 数据(代码+migration 已上生产,待挑第一批潜客试跑)**：方案见 [docs/specs/2026-08-24-report-page-discovery-upgrade-design.md](./specs/2026-08-24-report-page-discovery-upgrade-design.md),设计阶段过子牙/魏征/板桥三方复审,实施后魏征二审又挑出 3 个真实 bug 并已修复(`usableDiscovery()` 判断门槛未导出导致 page.tsx 各写一份、独立 admin route 缺每日成本硬顶、claim 查询用 `.in()` 塞 `null` 在 PostgREST 里匹配不到 NULL 行导致每个潜客首次调用必失败+`truncated` 状态不在可认领白名单导致永久卡死)。改动:`outbound_prospects` 新增 `discovery_report`+`discovery_report_status` 两列(migration `20260824000001`,**2026-08-24 已 apply 到生产 glbdnayojixmexgofbsd,验证过 2932 行老数据两列全 NULL、未受影响**)、`report.ts`/`page.tsx`/新 admin route `POST /api/admin/prospecting/discovery-upgrade`,181 个测试(prospecting 目录 167 个含 7 条新增 + discovery-upgrade 新路由 14 个)全过、tsc/eslint 干净。**下一步**:挑第一批潜客(闽商 18 家起)调这个新 route 跑试点,FDE 看完新旧对比给三选一结论。核心:不整页换成 `ReportView` 深色仪表盘,只把背后数据从轻量 `ProspectAnalysis`(~$0.15)换成完整版 `DiscoveryReport`(~$0.57);健康分继续用旧规则漏斗,discovery 六支柱分不作为第二个头部大字。**PM 拍板**:先跑 30 家试点(闽商 18 家 + 后续 12 家 qualified,`DAILY_DISCOVERY_CAP=30` 已写进代码),验收指标 = FDE 主观判断三选一结论;核验徽章带"NZBN 公开登记"来源说明;竞品对比是匿名行业均值,不点名。独立 admin route 不进 `prospecting-sweep` cron 循环。
- [ ] **P35.15 `/report/[id]` 报告页出中文版**（PM 拍板 2026-08-24，排在 P35.14 之后）：现状纯英文，面向新西兰本地商家通用场景；这次给 18 家闽商发的中文冷邮件因此没放报告页链接。需要方案：中英双语切换还是按收件人语言单独生成，与 P35.14 的数据源升级各自独立、互不阻塞。
...
- [ ] **⭐ IMPACT 闭环审计：8 个候选 ME 服务 0 个跑通 Check+Tune（2026-08-24，待同步 Build Control 统一排期）**：起因是闽商 outbound prospecting 项目整理「哪些服务能整合进 ME、非劳动密集型」的服务矩阵时，PM 追问「这些功能 IMPACT 都可以自动跑通吗」——核对后发现全部 8 个候选服务都只做到 Prescribe→Act（能自动执行动作），没有一个真正闭合 Check（结果验证）+ Tune（学习反哺下一次决策），不满足 CLAUDE.md 里「没有进入 Check 和 Tune 的 DAPE Execution 只能称为执行完成，不能称为 IMPACT 完成」的定义。转达 Build Control 前，本窗口没有可用通道（之前转达过 HOLD 指令的中转会话已下线，ListAgents 查不到；`priceless-margulis-59d637-76` 不再可达），先写进此处，任何窗口接手时请一并同步。三条需要排期判断的具体事项：
  - **`expected_metric: null` 是活 bug、不是「还没做」**：`src/lib/cms/seo-fix-orchestrator.ts:203` 和 `src/lib/cms/github-page-upgrade-publisher.ts:149` 里 SEO fix 类 `flywheel_actions` 都显式把 `expected_metric` 写成 `null`。`src/lib/flywheel/execution-outcomes.ts` 的归因 cron 靠这个字段找到要验证哪个指标——为 `null` 就永远进不了 attribution → `flywheel_outcomes` → memory-extractor 这条链，等于设计上把这类动作从飞轮里摘掉了，不是接线没做完。
  - **GBP 相关服务同理**：审计时确认是显式 `skip, no flywheel ingest`，跟上面是同一类问题。
  - **GEO 探针模型已废弃，静默降级（已独立 spawn_task 出去，task_257c892c，尚未修）**：`src/lib/ai-tracker/runners/openai.ts:17` 硬编码 `SEARCH_MODEL = 'gpt-4o-search-preview'`，该模型已于 2026-07-23 废弃（404），调用失败后静默返回 null 而不是报错。这是本次审计的副产品，影响面大概率不止闽商试跑，是全部客户的 AI Visibility Tracker 共用的 runner。
  - 另有一处已知产品缺口顺带记录：outreach 文案里在推的「$99 一页站」目前没有后端生成器，纯 offer 没有实现，不属于 IMPACT 闭环问题但同样需要排期判断。
