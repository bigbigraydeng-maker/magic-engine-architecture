...
- [ ] **P3 待认领**：PR #1612（`docs/sops/client-meta-ads-onboarding.md` 通用版 SOP）复审卡住未合，8 条 Codex 意见未解决（令牌环境变量命名规则对不上代码实际读取逻辑、系统用户跨客户共用有泄露风险、缺 `leads_retrieval` 权限会导致线索收不到、客户授权等待态没接入统一人工待办管道、Page 专用令牌被广告调用误用等），多数标 P1 级但暂不逐条修——2026-09-15 PM 按 §11 三维打分拍板 P3：频率低（只影响 Magic Picks 一个客户）、IMPACT 关联低（文档还没合并生效，不影响任何在跑流程）、收入关联低（不是天天用到的路径）。谁接手时从 PR #1612 的复审对话逐条看，8 条意见原文在 GitHub PR 页面
