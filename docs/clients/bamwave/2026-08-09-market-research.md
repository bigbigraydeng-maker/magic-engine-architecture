...
> - `2026-08-09` 三次核查：**更正 §2 的一处误判** —— 早先写的「外网出口整个是关着的」不成立（对照测试 `github.com` 是通的）。真实原因是白名单太窄 + **旧会话读不到后加的域名**，解法从「排查网络」改成「加完域名后开新会话」。仅改 §2，其余内容未动。
...
> **更正**（2026-08-09 第三次核查）：本节早先写的「外网出口整个是关着的」**不准确**。实测 `github.com` 是通的（见下表），所以出口没有全关，是白名单太窄。这个区别很重要 —— 它意味着**不需要排查网络故障，只需要把域名加进名单再开新会话**。
...
| PM 把 `bamwave.co.nz` 加进名单后再测 | **仍然 403** | 设置保存成功，但**旧会话的名单在容器启动时就锁死了**，读不到新增项 |
...
- **A**：放行 `bamwave.co.nz`，**然后开一个全新会话**
  → 打开 **https://claude.ai/settings/code** → 找到本项目的环境 → 网络访问设置 → 把该域名加进允许清单
  → **关键：域名已经加过了，问题不在设置，在于生效时机。** 环境的网络名单是**容器启动那一刻锁定**的，已经在跑的会话读不到后加的域名 —— 所以**必须开一个全新会话**才会生效，在旧会话里反复试是没用的（已实测两次，均失败）。
