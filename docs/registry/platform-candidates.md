...
| Paid Search Ads Discipline Playbook（6 子规则：Brand/Non-brand 分账 · Broad match 谨慎 · 主题化 ad group + Smart Bidding 学习期数学门槛 · Conv value 真实客单价 · Heritage 词并列本地经营年数 · Change History 是 audit 第一步）| L1 Capability | 广告 支柱（主）+ Measurement Contract 平台基础设施（次·conv value 真值规则）· IMPACT Prescribe/Act/Check 段 | CTS Tours NZ | Outbound Tourism (China 线) | 1/3 客户复制（仅 CTS · 需 Roman 中介广告 + Oztop/Magic Picks 电商广告 补齐硬证据）| candidate | 2026-08-29 | 2026-10-29 | 主 session（CTS $40/day 重启优化） | 来源 CTS Google Ads $40/day 重启：Path B → 子牙对抗性复审 4 条挑刺 → Ray 反馈"4 paused ad group 当时没效果"→ 抽出通用规则。张良 v2.1 Full Report 修正 PM 原判"1-3 通用 / 4-5 旅游特有"→ 6 条全 L1 通用（4 是 lead-gen 通用陷阱、5 是跨国品牌+本地分公司通用规则、6 是本 window 新加）。**实现形态 = Hybrid**：Skill 部分已建 [`.claude/skills/me-google-ads-discipline/`](../../.claude/skills/me-google-ads-discipline/SKILL.md)（v0.1 EXPERIMENTAL · PM 2026-08-29 明确拍板"现在建骨架"，已告知抽象过度风险）；Agent 部分（`google-ads-auditor` cron · 规则 4+6 周期巡检）**未建**，等硬证据 + 五道 Build Gate + 2 审。相关 issue [#1216](https://github.com/bigbigraydeng-maker/magic-engine/issues/1216) 打包同一支柱；hardcoded 客户名 / 行业 = ✗（走 `clients` + `master_briefs`）|
