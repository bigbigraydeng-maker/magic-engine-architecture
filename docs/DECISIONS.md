...
> 🔴 **实施变更（子牙复审拍板）：组 R SEVER 已折进 ai-tracker 退役同一个 PR**（不再单独小 PR——PO 要减法要快、且两者架构不冲突）。**因此产生一处 Goals 行为变更,必须让 PO/未来的人一眼看到,别埋在大 diff 里**：**自本 PR 起,Goals 的 `ai_visibility_score` 失去自动来源、`autoFetchMetricValue` 对它返回 `ok:false, severed:true`,current_value 留空（refresh cron 会主动清掉旧的行业均值）**,直到 M1 按客户实测接上（P31.X.4）。**这是有意的**——它原先自动填的是系统 C 的行业均值冒充客户分,摘掉一个错数字留一个诚实的空,**不是 bug、不是无损 no-op**。`baseline-audit.ts` 的 `AUTO_METRICS` 同步去掉 `ai_visibility_score`（美化:否则清单里显示「auto 但拉不到」）。

**完整拆除清单**（逐个缠线定性 DELETE / REWIRE→M1 / LEAVE-SEVER、删除执行顺序、DROP 表清单、我方异议）见 [`docs/specs/2026-08-19-ai-tracker-decommission-v1.md`](./specs/2026-08-19-ai-tracker-decommission-v1.md)。

> **实施状态（本 PR #1077 真实状态，取代原「零删除规划稿」措辞）**：本 PR **已删** ai-tracker 代码 / API 路由 / dashboard 页 / `ai-tracker-weekly` cron（含 `render.yaml` + `cron/registry.ts` + `STATE.md`），共享件平移到 `src/lib/ai-probe/`，组 F/H/I/K/L/M/N/J 降级为「未测量」，组 R SEVER 已折入。**唯一没做的是 DROP 三张表**——三表现为零代码引用的孤表，DROP 是合并后 PO 独立 `go apply`（归档已随本 PR 落在 `docs/clients/cts/`，是 DROP 硬前置）。
