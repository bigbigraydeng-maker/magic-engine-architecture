...
**顺手查出**：全仓还有约 44 个迁移文件、约 50 张表带同款旧模板，全部早于 08-03、大概率都已被那次扫描一并修复。这堆源码本身值不值得逐个补（收益低、纯卫生），以及要不要加一道 CI 检查防止未来复制这个坏模板，留给后续决定。

> ⚠️ **纠正**：本条最初的版本写了「08-03 之后没有再犯」——**这句话是错的**，Codex 复审这条 PR 时当场抓出反例：`20260803140000_client_projects.sql`（建于 08-03 那次扫描**之后**）用的正是同一个漏 `TO service_role` 的旧模板，且从未被后续任何文件修过。PM 核实生产现状已是 `roles={service_role}`（原因不明，可能是人工改过，也可能是实际 apply 顺序跟文件时间戳不一致——账本会在 apply 时重编号，见 [STATE.md:86](../STATE.md)），不是活漏洞，但同样需要补源码，另开 PR 修复（`20260819000001_fix_client_projects_rls_service_role.sql`）。教训：「早于 08-03 = 已被扫描覆盖」这个假设**只对建表时间早于扫描时间的表成立**，晚于扫描时间的新表必须逐个核实，不能类推。
