...
这个人的档案。⚠️ 但这条锚点**只对第一条入站消息之后的消息无条件成立**：
第一条走的是「没有问候语也认，只要凑够两条标准字段」的兜底（Meta 模板措辞会随
语言变，卡死问候语会漏掉真表单）。所以**第一条消息正好是转发的同行者资料**时，
这层保护不在 —— 缺口已登记，见 ROADMAP M2.7p。
...
**没做的五条**已登记到 [ROADMAP § Phase 24.M「M2.7p」](../ROADMAP.md)（本文件只记已上线的事）。
