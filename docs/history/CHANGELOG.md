...
### 2026-09-07（销售的客户需求卡停更 14 天：`&&` 守错了信号，改走 Inngest）

**修的是什么**：给销售写「客户需求卡」的那条任务从 **2026-08-23 00:12 起 14 天一次都没跑过**（[#1440](https://github.com/bigbigraydeng-maker/magic-engine/pull/1440) 的对账测试顺带挖出来的，当时登记为 TD.15「原因待人工确认」）。原因这次查到底了，**不在 Render 后台，在本仓的 `render.yaml` 里**：

`messenger-hourly` 那条服务的启动命令是 `curl 私信同步 && curl 写需求卡`。意图是对的 ——「同步没跑成，就别拿半截数据写卡给销售看」—— 但 **`&&` 守的是 curl 的退出码，而退出码回答的是「网关有没有在超时前把响应给我」，不是「同步有没有跑完」**。两者从 2026-08-17 07:10 起分家：那一轮起私信同步的服务端耗时从约 25 秒跳到约 140 秒，网关掐断返 524 → `curl -f` 退出码 22 → `&&` 短路 → 第二条 curl 从此一次都没执行。服务端每一轮都跑完了、每一轮都写了运行记录，所以监控上「私信同步每小时正常」。

**先把机制钉死才动手**（生产库 `cron_run_logs` 501 条运行记录实测）：闸门不在 100 秒，在**约 125 秒**。跟着跑成功的那些，同步耗时最大 124.5 秒；没跟着跑的，最小 124.98 秒 —— 零反例。这正好解释了 8/21（9 次）和 8/22（16 次）为什么还有卡：那两天同步刚好快到 119~128 秒，反复在那条线上下浮动，落在线内的小时就写了卡；8/23 01:10 之后再没低于过 125 秒，于是彻底停。

**为什么慢的**：2026-08-17 06:00 那个提交（`628c7b2e`「点私信开场白里的电话邮箱补进档案」）给同步加了一条**按人逐个查**的存量扫描 —— CTS + Roman 共 314 个候选人，每人两次查询，**628 次串行往返**，把这条任务从 25 秒推到 140 秒。

**改了三件事**：
1. **判据换成真的**：同步路由跑到最后一行发一张 `me/messenger.sync.completed` 条子（带服务端自己的回执：几个客户 / 几条消息 / 失败几个），Inngest 上的 `cloud-messenger-brief-after-sync` 收到就写卡。响应有没有在网关那条线以内送达，跟写不写卡**完全无关**。同步中途整个挂掉走不到那一行 → 卡照旧不写，原来那个意图完整保留。
2. **写卡改成分段跑**：整轮 160~210 秒，一个请求装不下。现在每段最多 12 张卡（实测每张约 3.2 秒 ≈ 40 秒一段），失败只重试那一段，已经写好的卡不重写。`/api/cron/messenger-brief-hourly` 保留为手动补触发，且**故意不写运行记录** —— 手动点一下写进去会把「人手补的」伪装成「定时链路正常」。
3. **把那 140 秒压回去**：存量扫描从「一个人查两次」改成「一批人查一次、在内存里分组」，往返次数从 600+ 降到个位数，判据一个字没改。顺带补上分页 —— Supabase 默认最多返回 1000 行**而且不报错**，批量查一不小心就会静默截断，截掉的正好可能是某人的开场白那条。

**顺手加的两道闸，防同一类事故再来一次**：
- `render.yaml` 里**任何 cron 都不许再用 `&&` 串第二个 `/api/cron/` 调用**（对账测试锁死，把第二条 curl 加回去当场红）。串在 `&&` 后面的健康检查上报不受影响 —— 它漏掉的方向是「误报没跑」，会响；漏掉一件活儿的方向是安静地不干，不会响。
- 清单的扫描器现在也认 `startCronRunId`。跨步骤跑的任务（Inngest 那批）开运行记录用的是它，原来只认 `startCronRun` —— 每周 SEO 快照和这次的私信简报都属于「写着运行记录却扫不出来」，跟「这个任务不存在」长得一模一样。

⚠️ **合并后必须做一步**：`curl -sS -X PUT https://app.magicengine.com.au/api/inngest`，返回 `modified:true` 才算真上线。Render 不是 Vercel，没有自动同步，新函数没 Sync 的表现是**安静地不跑**。这条链路已登记进 `CRON_REGISTRY`（`scheduler: 'inngest'`，故意不填 `addedAt` 宽限期），真没跑起来健康检查会喊「过期没跑」。

**两份复审（子牙架构 + 魏征变异检验）抓到的必改项，全部已修**：
- **`retries: 0` 写反了**（子牙）。查了 Inngest 3.54.0 的类型定义：`retries` 是「**这个函数所有 step 的最大重试次数**」，不是「整条重试次数」。写 0 = 任何一段抛错整条立刻判死 —— 第 3 段挂了第 4、5 段全不跑，更要命的是收尾那步（一次 Supabase update）也零重试，网络抖一下那行 `cron_run_logs` 就**永远停在「在跑」**，而这条链路的全部立论就是「那些记录是真的」。「重试会重复花钱」的担心也不成立：每段是独立 memoized step，重试只重跑挂掉那一段。改成 `retries: 2`，跟同仓样板一致（多段编排用 2，单段干活的 worker 才用 0，第一版把 worker 的值抄到了编排上）。
...
**顺带查出的生产事故**：`messenger-brief-hourly`（给销售写客户需求卡）**从 2026-08-23 00:12 起停了 14 天**。它是 `messenger-hourly` 那条 Render 服务里的第二条 curl，而清单原来一条服务只登记一个 `jobName`，第二条就此隐形。不是记录丢了 —— `conversation_briefs` 整张表最后被写入也停在 `2026-08-23T00:12:37`。同一条服务的第一条 curl（私信同步）945 次每小时都在跑。~~停因在 Render 那一侧（本仓这段配置自 2026-07-27 未动），代码侧查不到，需人工看那条服务的运行日志。~~ **这句判断错了**（2026-09-07 同日更正）：停因就在本仓 `render.yaml` 那两条 curl 之间的 `&&` 上 —— 配置确实没动，是**同步变慢**让那个一直存在的判据错误第一次显形。详见本文件同日「`&&` 守错了信号」那条。
...
- **恢复条件（满足任一就把这条链事件化）**：① `attribution-cron` 出现单次超过 25 分钟或连续失败；② 抽取器读到的 outcome 明显滞后于 attribution 那一轮（学到残缺数据）；③ 这条链上再接第四个环节。届时改为 `attribution` 完成后发 `flywheel.outcomes.computed`（带 run id / client 数 / outcome 数），由 Inngest 接力触发抽取与汇总。
...
问过 PM 要哪种呈现，选的是"时间轴/计划表"，但系统里**没有真实排期数据**——
硬编日期就是编数字，违反"不装懂"铁律。改成按组件依赖的拓扑深度排序：
越靠前的越是别人要先等它的地基，不给假的"预计几号完成"。

**做了什么**：`presenter.ts` 复用既有 `layerByDepth()` 拓扑深度计算（跟依赖图
tab 同一套 `requires/consumes/implements/adapts` 口径），加一个「接下来要做什么」
tab，列出还没到最终成熟度的组件，附当前状态→下一步、拦路虎摘要、是否等 PM 拍板。
页面文案明确写"这不是日历"。

子牙（架构）+ 魏征（挑刺，实跑测试）两轮复审均 APPROVE。353 测试 + build 通过。

---

### 2026-08-17（客人在私信里说「别再联系」，终于有人看得见了 —— 但刻意只提示，不自动封）

**缺口**（issue [#1025](https://github.com/bigbigraydeng-maker/magic-engine/issues/1025)）：
私信正文早就同步进 `conversation_messages` 了，但**判「他还想不想被联系」的那一步
从来没去看它** —— `classifyNote` 全仓唯一的生产调用点是销售手打备注那条路。
于是邮件渠道说的「别再联系」看得见（`mail-ingest` 写了 `raw`），**私信说的看不见**，
而私信恰恰是 CTS 客人说话最多的渠道（线上 2099 条 / 658 个会话）。

**PM 2026-08-17 在知情下选的是 B**：先接内容、**只用来提示销售，不自动封渠道**。
理由是判据本身现在还判不准（[#1019](https://github.com/bigbigraydeng-maker/magic-engine/issues/1019)
就是为它开的）—— 旅游生意里同一个词的意思是反的：`take me off` 是要求下车、
`opt out` 是不参加自费项目、`all sorted` 是已经在我们这订了。一次性把 2099 条私信
喂进一个会误判的判据，代价是**一批正常客人被永久静默排除**，解除只能一个个手动点。

于是把风险方向倒过来：判中了只下发人工任务，**判错了浪费销售 10 秒，判漏了跟今天一样**。

**做了什么**：`lib/crm/messenger-stop-signal.ts` —— **一行写操作都没有**，这是它的
核心纪律。命中的人进今日待办「🙋 需要你动手」（新条目 `dm_maybe_stop`），
带客人原话摘录 + 直达那个人的链接。判据仍是同一个 `classifyNote`（一份判据）。

**四条收口，每条都有变异验证钉住**（故意拆掉，对应用例立刻红）：
已经算拒联的人不提 · 私信之后有人手工记过一笔就不提（有人看过了）·
同一个人只提最新那一句 · 上限按**客户**算、被压掉多少必须 log 出来。
「人纠正过判错了」的人照常提示 —— 判据只走 `isDoNotContact` 那一份。

**两条已知代价，如实记**：只看最近 30 天（更旧的不再提起）· 每客户每轮最多 20 条。
要做到「一条不漏又不重复骚扰」，前提是加一列「已复核」（改 schema，A 级），已登记。

---

### 2026-08-17（Facebook 私信里留的电话邮箱，终于补进客户档案了）

**PM 报的现象**：CRM 卡片上写着「💬 没留电话 —— 只能在 Messenger 回他」，
**但那个人明明留了电话和邮箱** —— 就在这段对话的第一条消息里。销售信了卡片
那句话跑去私信回，而客人在等电话。

**真因不是读不到**：Meta lead 广告走 Messenger 时，客人填完表单会以他自己的
身份发出一条固定格式开场白（`Full name:` / `Phone number:` / `Email:`），
正文早就同步进 `conversation_messages` 了。问题是判据从来没读它 ——
`matchConversationToContact` 只拿正文里的邮箱去「认出这是谁」，认完就丢，
**从不把电话邮箱取出来补进档案**。于是 `contacts` 那两栏一直空着。

**做了什么**（PR [#1031](https://github.com/bigbigraydeng-maker/magic-engine/pull/1031)，
合并提交 `580b0311`）——三条路共用同一份判据：

- `lib/crm/messenger-lead-intro.ts` 纯解析（标准化复用 Meta 表单的
  `parseLeadAnswers`，**一份判据两个入口**，以后表单加字段两边一起认）
- `link-contacts` 实时接线
- `lib/messenger/lead-intro-backfill.ts` 按**人**扫存量（谁的档案还缺电话或
  邮箱就翻他的私信），跑在 Meta 凭证闸**之前** —— 它只读本地库，授权断掉的
  客户恰恰最需要它还在跑

**五条纪律**（十轮复审逐条收出来的，每条都有变异验证钉住）：
只认客人自己发的那条 · 只补空栏绝不覆盖 · 号码归谁先查清楚 ·
身份没写成就不写档案 · 按字段各取最新非空值。

模板判据认那句**锚在消息开头、主语是「我」**的问候语。「my friend filled out
the form」不算 —— 那是转发同行者资料最自然的说法，认了就把**别人的号码**写进
这个人的档案。没有问候语时（Meta 模板措辞会随语言变，卡死它会漏掉真表单）走
兜底：**姓名 + 电话 + 邮箱三条全齐**才认。
电话按 `clients.country` 补国码，认不出市场就不写本地号（Oztop 是 AU，
按 NZ 补会写出一个陌生人的号码）。

**当天补的一刀（PR [#1034](https://github.com/bigbigraydeng-maker/magic-engine/pull/1034)）**：
上面那条兜底原先只要求**两条**字段。Codex 复审 PR #1033 指出，调用方对**第一条**
入站消息不要求问候语，于是「`my friend filled out the form` + `Name:` + `Phone:`」
只要恰好落在对话第一条，就能冒充开场白，**把别人的号码写进这个人的档案**。
门槛提到三条全齐 —— 真表单同时收姓名 / 电话 / 邮箱，换了语言也凑得齐；
转发一段同行者资料通常凑不齐。变异验证钉住：门槛改回两条，新增的两条用例立刻红。
⚠️ 仍不是密不透风：转发的资料三样俱全照样能过，写入侧的空栏守卫与号码归属回查
是最后一道 —— 判据这一层只做到「宁可少认」。

**过程中的一次教训**：PM 在对话里贴的线上原文（真实姓名/电话/邮箱）被我原样
写进了测试和注释并推上远端。已全部换成虚构值；因仓库禁用 squash 合并，改为
从 main 起干净分支重开（#1029 关闭），那几条提交没有进 main 的历史。

**没做的四条**已登记到 [ROADMAP § Phase 24.M「M2.7p」](../ROADMAP.md)（本文件只记已上线的事）。
...
### 2026-08-16（让系统读那 90% 从没读过的对话）

PR [#997](https://github.com/bigbigraydeng-maker/magic-engine/pull/997)。

**系统只读了 10% 的对话，就在回答「这个人到哪一步了」。**

线上实测：判断只看**电话手记 295 条**，而**邮件正文 523 条、私信 2099 条**
两个月前就同步进库了，从来没有任何东西读过它们 —— 私信原文在
`conversation_messages` 里，触点表只写了一句「Messenger 私信（N 条往来）」。

所以 583 个人里 **556 个的「跟进到哪步」是空的**。不是规则不够细，是没读过他们说的话。
PM 2026-08-16 拍板：读。

做法：判断在 `lib/crm/stage-from-conversation`，取数落库在 `lib/crm/stage-infer`，
挂在已经在跑的 `messenger-brief-hourly` 上跑第三遍（不新注册 cron —— 新 cron
要手工 link 密钥的环境变量组，漏了就每天静默 401）。**不引入任何新数据源**：
读的全是我们自己的号早就同步进来的东西，不连客人的邮箱。

三条不许破的线：

- **只填空着的** —— 候选人只从 `stage IS NULL` 里挑，写库那一刻 UPDATE 的 WHERE
  里再判一次。销售手标过的阶段是这套系统里最硬的信号，结构上就没有覆盖的可能
- **说不出逐字原话就整条丢掉** —— 模型必须交回一句来自对话的原话，回原文里核对，
  对不上整条不算数。空着只是没帮上忙，编一句「他说想订十月的团」是伤害客户
- **终结档和涉及钱的档不接** —— 「不感兴趣」落进去这个人就从所有名单上消失；
  一封写着「明天转账」的邮件不等于钱到账。模型只允许落在还会继续跟的那几档

每填一个都在时间线上留痕：为什么这么判 + 依据的原话，`changed_by` 写
`ai:conversation-read`（跟人工改阶段永远分得开）。销售不服可以直接改。

**读不出来的人进「新线索」** —— PM 拍的一条简化，也是整件事的关键。原先读不出来
就什么都不写，于是这个人每天被重读一遍、每次都读不出来（对话太薄的人永远读不
出来）。为了不重复烧钱，一度打算加一列记「上次什么时候试过」；PM 一句话点破：
**读不出来本来就该进潜在客户池**，那才是他现在真实的位置。写下来他就不在候选里了 ——
那一列不用加，候选队列的轮转逻辑也一并删掉。

同一轮还修了「取消『别再联系』」那条路上的三个死胡同：纠正之后分段仍按历史
拒联判 `excluded`（人不回名单且再没按钮）· 今日名单会立刻建议把刚纠正的人
再埋一次 · 还停在「停止营销」那一档时默不作声。

---

### 2026-08-16（号码打不通 ≠ 这个人不要了；「暂时不考虑」≠「明确不要了」）

PR [#995](https://github.com/bigbigraydeng-maker/magic-engine/pull/995)（`79873161`）+
[#996](https://github.com/bigbigraydeng-maker/magic-engine/pull/996)（`c9b466f2`）。

起点是 PM 的一张线上截图：「Sue Masson 我们一直在跟她邮件沟通，怎么归到
『号码是坏的』那一栏去了？」查下去发现的不是一个人的问题 ——

**24 个被标「号码是坏的」的人里，23 个后来又来过消息，15 个一直在跟我们邮件往来。**
Sue Masson 7 月 6 号被标坏号，此后来了 11 封信、最后一封是前一天，
全程躺在一个「已排除」的栏里没人回。

症结：`bad_number` 被当成了**结局**，跟「他说不买了」同级。**一个渠道故障判了整个人
的死刑**。而同一个病还有第二个变种：系统只有一个终结性的「不感兴趣」，
一句「明年再说」会让人从此不出现在任何名单上（线上 17 人）。

判据分层，这次立了两条通则：

> **联系方式是渠道属性，成不成是人的状态 —— 两件事不许互相覆盖。**
> **一个兜底值不许推翻一个客人明确表达过的结论。**

第二条当天撞了四次，每次都是同一种伤害：拨到空号被算成「今天跟进过他了」
（待办被折叠藏起来）；手打笔记里的兜底「聊过了」把坏号判活（那个号又变回可拨）；
系统自己群发的那封邮件抹掉客人刚说的「现在先不考虑」（第二天又被推回真人通话名单）。

一并焊死的还有「同一个判断只允许存在一份」：坏号判据、失败拨打判据、客人结论判据
各自导出一份共用函数 —— 今天三次栽在「同一件事在不同读模型里各写一套」上
（分段说「打不通」、分组却说「不要再联系」，补号码那件该有人动手的事又被藏起来）。

存量数据**读的时候重判一次**，不回填：原话是事实，`outcome` 只是我们对它的解读，
解读可以更新。三条读模型（今天名单 / 全部客人 / 时间线）共用同一个函数，不会说两样话。

**刻意没做自动化的一件事**：旧词表把「不打算去」当成过「别再联系」，线上有 1 个人
可能被误标成永久拉黑。**不写自动解除** —— 那个标记的含义是「任何渠道都不许再发」，
让正则去解开它，万一原话里同时有真拒绝，就会骚扰一个明确说过别联系的客人。
下发成人工任务（what / how / href 齐全，见 ROADMAP M2.7l）。

700 条 CRM 测试 + 全程变异验证（每一处判据都亲手改坏、确认有测试抓住）。
Codex 八轮复审：绝大多数采纳，两条写明不改的理由。最后两轮改的是**判法**不是加词 ——
「说定了的下一次通话压过含糊的『现在还不…』」「当下的购买结论压过之前的犹豫」——
一条规则结清一整族。

⚠️ **留给下一个人的结论**：软硬拒绝的区分**正则做到头了**。八轮里每收窄一次就冒一个
新说法（`ready` → `yet` → `going` → `考虑一下` …），这不是规则不够细，是语义判断。
本仓在 `lib/crm/next-step` 已经得出过同样结论。下一步是让解析器直接回答，
并解除 `parseNote` 对规则结果的无条件覆盖 —— 见 ROADMAP M2.7h。

---

### 2026-08-15（卡片上直接敲一行 —— 说个时间，系统自己排上下一步）

PR [#989](https://github.com/bigbigraydeng-maker/magic-engine/pull/989)（合并提交 `a097a7f5`）。

板桥（销售视角复审）说他最想要的就是这个，而且**比三段式改版更重要**：
「我一手拿电话一手用鼠标，不可能点开抽屉、再点四下、再从下拉里选一个阶段。
我要的是：电话一接通，卡上开个输入框，我敲一行『三月两个人去南岛，周五给报价』，
回车 —— 系统自己在周五那天的名单上给我生成一张卡。」

排程那一半在 #985 就通了。**这次做的是卡上那个输入框，和存完之后那句确认。**

关键不在输入框，在那句确认（`lib/crm/next-step.ts`）。只回一句「记好了」的话，
销售**无法确认那个「周五」被读懂了没有** —— 他只能自己再记一遍（功能白做），
或者信了而周五没人提醒（**比白做更糟，答应客人的事砸了**）。所以它必须分得清三种结局：
读懂了 → **说出哪天、带星期几**（他嘴里说的是「周五」）· 没说下一步 → 别出声 ·
**说了但没读懂 → 明说没排上**，绝不假装排好了。

**「有没有约下一步」最后交给解析器答，不用正则。** 正则版当天被改了四轮：
只看日期 → 把客人的出行日期误报成回访；整句排除「出发」→ 把「客户十月出发，
周五联系」里那个**真的回访日期**一起屏蔽（漏报，比误报危险）；改成按子句判；
「刚给他报价了」（已经做完的）照样误报。第四轮说明问题不在规则不够细 ——
这是语义判断，所以让本来就在读这句话的解析器多答一个是/否（`mentioned_next_step`），
正则降级成没配 API key 时的兜底。**别再往它上面加规则。**

三条焊死的交互：**回车提交、Shift+回车换行、Esc 收起**（他另一只手在拿电话）·
**打中文选字的那一下回车不算提交**（不挡的话笔记会在只打了半句时存下去、输入框
当场收起，而他多半不会重打一遍）· **方向能切**（「客户打回来了」的搜索结果用的是
同一张卡，固定写成出站会把一个还在等回复的客人当场标成已处理）。

防重键跟着**这一次要存的内容**走（方向 + 原文）：同样的内容重试复用（双击不记两笔），
**内容或方向改过就换新**（否则改过的那版会被服务端当成重复整个丢掉，而服务端是
先解析新文本、去重之后照样更新联系人 —— 新文本里那句「别再联系」会改掉这个人的
状态却没有任何触点记着它）。

同时把 vitest 的 JSX 运行时对齐到 Next 的生产配置，顺带修好 5 条早就红着的组件测试。

Codex 复审十一条：九改、两条写明不改的理由（未到期的约定要不要把人暂时收起来是
**生意决策**，已上抛 PM；重试时确认降级已登记为后续小 PR，笔记和排期都没丢）。

---

### 2026-08-15（页面里回完私信，卡片当场变灰）

PR [#988](https://github.com/bigbigraydeng-maker/magic-engine/pull/988)（合并提交 `df2c818c`）。

`messenger/send.ts` 发出去之后只写了会话消息和审计，**没写出站触点** —— 而「今天
处理过没有」是从触点算的。于是在页面里回完私信，卡片最长要等一小时（等每小时同步）
才变灰：正是 PM 抱怨的那个毛病换了个渠道再犯一次。

发送成功后当场补写，**幂等键跟每小时同步用的是同一个**（`source='messenger'` +
`source_ref='<convId>:out'`），所以同步再跑不会把同一条记成两笔。时间**只许往前推**
（`advanceOutboundTouch`：先 `ignoreDuplicates` 插入、再只对更早的那条 `UPDATE ... < at`，
两条原子语句，不做「先读再写」—— 那样两个并发请求会互相覆盖）。

---

### 2026-08-15（今天的名单一天之内不变 —— 做完的就地变灰，不再跳组消失）

PR [#985](https://github.com/bigbigraydeng-maker/magic-engine/pull/985)（合并提交 `096111a7`）。

**这次解决的一件事**：PM 两次实测同一个毛病 ——「每次点卡片，做了动作后回到目录页，
我如何知道哪个已经联系了？当前的设计无法做成工作流水线。」

症结不在显示（灰卡和计数 8/2 就做好了），在**分批是实时算的**：一记录「联系过了」，
`lastOutbound` 从 0 变非 0，这个人当场不属于「新客人」那一批 —— 他不是变灰，是换位置
或干脆消失。对一条流水线来说这是最致命的：**你脚下的地面在动。**

判据在 `lib/crm/day-list.ts`：**只冻结「我们做的动作」，不冻结「客人做的动作」**，
只增不减、不重排。不新建状态列 —— 这套系统最贵的教训就是手工状态列（CTS 那份
128 行手工 CRM，阶段列 0 个填了），「今天处理过没有」全部从已有事实推。

三块：① 冻结 / 灰卡 / 进度条（原 PR #855 被清理关掉，从未上线，本次取回含 21 条测试）
② 推迟和推到成交不再点完就消失（原 M2.7a；证据是那一笔触点的 `action` 标记 +
`contact_stage_events.from_stage`，**不加列**）③ 告诉解析器今天几号 —— 销售说的
下一步基本都是相对的（「周五」「下周二」），不知道今天是哪天就一条都排不上。

承重墙：改之前**在不在名单上**才决定清不清（已成交的人不许被塞回名单）·
管理动作（推迟 / 取消推迟）不算「我们出手了」，不许盖住客人当天的回信 ·
承重的留痕先写后改库，失败时偏向「让人留在名单上」（多一个人是噪音，少一个是丢单）·
换季那两天按当地午夜取时差（原写法在当地中午取，一年错两天、每次错一小时）。

553 条 CRM 测试 + 19 处变异探针（其中 1 处挖出真实测试盲区，已补）·
全量套件逐条对基线零新增失败 · Codex 六轮 15 条：13 改，2 条写明不改的理由
（「别再联系」当天消失是刻意的 —— 客户数据红线；「给已推迟的人延期」界面不可达，
已在代码里标注雷点和修法）。
