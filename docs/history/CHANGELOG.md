...
**魏征挑出的四条缺口**：① RLS 从一开始就显式写 `FOR ALL TO service_role`（`outbound_prospects` 当年漏了这行，酿成 2026-08-03 那次 2,678 行泄露事故——这次没有重蹈，已用匿名 key 探针实测确认）；② 认证状态字段写了 `assertOfficialSourced()` 兜底——`source_url` 是空字符串时，"verified" 会被降级为 "unknown"；但这只是"非空字符串"校验，不核实 URL 是否真的指向 Meta/Google/TikTok 官方渠道，多数 source_url 其实是候选公司自己网站的宣传页而不是官方目录，这一条本就是弱校验，不是硬约束——spec §6 本身也允许"公司资质页展示真实徽章"这种证据档，不强制只认官方目录；③ 合规页脚重新写了 `partnerComplianceFooter()`，不能照抄 prospecting 那句"通过你的 Google Business 列表找到你"（渠道伙伴不是本地商家，来源措辞会变成谎言）；④ `domain` 加了唯一索引兜底"第一轮最多联系一次"。**Codex 复审后又挑出第 ①.5 条**：`assertOfficialSourced()` 写完只顾着测试，没有任何写入路径真正调用它——本批 24 条种子数据手工核对过（14 条 verified 全部带 source_url，无漏洞），但代码层面本不该靠"手工核对凑巧对了"，已补 `sanitizePartnerCandidate()` 作为强制关卡，未来任何写入路径都必须先过这一步。

**研究阶段**：三个平台并行研究出 24 家去重后的 AU/NZ 真实候选（Meta 8 / Google 10 / TikTok 7，Soul+Wolf 跨平台重复合并），全部诚实标注 verified/unverified——Meta 官方 Partner Directory 需要登录态，公开工具打不开，8 家全部如实标 unverified，没有为了凑数字编造资质。打分阶段发现一个值得记的事实：spec 自己的权重设计里 escalation/training/branding 三项占 50 分，这些信号只有等对方回信才能确认，调研阶段全是 unknown 拿不到分——所以没有一家在这一轮就能到 Priority A/B/C（最高 42 分），这是反幻觉设计的必然结果，不是 bug。Wave 1 选出的 10 家入选依据不是单一标准（Codex 复审纠正）：Globital、Blue Water Digital、Four Dots、SimplySEO 这 4 家是页面明确写了白标/代理商合作项目（`b2b_partnership`/`white_label` 都标 `yes`）；SearchMax、Unbound、The Pistol、Alpha Digital、We The People、Growth Huntr 这 6 家白标/代理商信号仍是 `unknown`，入选靠的是另一类信号——Google Premier Partner（top 3%）资质或 TikTok 官方 Partner Directory/Badged Agency 收录。所有 10 家都有 `source_url` 可查，但认证强度参差不齐——少数是官方合作伙伴目录页（如 `partners.tiktok.com`），多数是候选公司自己网站的宣传页，不代表真的经平台官方核实过。
...
**分支小插曲**：最初提交在 `claude/trusting-leavitt-231732`（自动修车道），因为改动 968 行 + 两个数据库 migration 触发了 `fix-scope` 闸门（超 800 行上限 + 数据库结构变更是受保护路径），照闸门提示换到 `feat/platform-partner-outreach` 分支重开 PR（#1317 关闭改到 #1318）合并；合并后 Codex 在收尾的 CHANGELOG PR 上又挑出上述三条真实问题（守卫未接线、Wave 1 依据混淆、L4 分类过度归类），一并修在这条记录里。
