...
**顺手查出、且核实范围比最初判断更小**：全仓还有约 44 个迁移文件、约 50 张表带同款旧模板。其中**只有 `20260803140000_client_projects.sql` 一张表建于 08-03 那次批量扫描（`20260803020000`）之后**，从未被后续任何文件修过——这条是 Codex 复审本 PR 时抓出的反例，直接推翻了「08-03 之后没有再犯」这个初稿结论。**除 `client_projects` 外，其余全部早于 08-03**，PM 核实生产现状后确认都已被那次扫描一并修复（不是活漏洞）；`client_projects` 的生产现状也已核实是 `roles={service_role}`（原因不明——可能人工改过，也可能实际 apply 顺序跟文件时间戳不一致，账本会在 apply 时重编号，见 [STATE.md:86](../STATE.md)），同样不是活漏洞，但源码问题是真的，已另开 PR 修复（`20260819000001_fix_client_projects_rls_service_role.sql`）。

**教训**：「建表早于 08-03 扫描 = 已被覆盖」这个假设**只对真早于扫描时间的表成立**，不能拿"当时旧模板还很普遍"来类推所有旧模板文件都安全——必须逐个核对文件时间戳。其余约 43 个文件本身值不值得逐个补源码（收益低、纯卫生），以及要不要加一道 CI 检查防止未来复制这个坏模板，留给后续决定。
