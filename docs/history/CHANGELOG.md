...
### 2026-08-19（ME2 Product Map「老板摘要」视图上线）

**发生了什么**：Product Map 控制台原来一屏摊着 ~50 个组件，看的人得自己在脑子里做汇总。现在多了一个**老板摘要**视图——按业务线收成一行一张卡片，一眼看出每条线「几个在跑 / 卡了几个 / 下一步是什么 / 有没有事等你拍板」。

**做了什么**（PR [#1076](https://github.com/bigbigraydeng-maker/magic-engine/pull/1076)）：新建 `src/lib/product-map/summary.ts`（纯函数、零 IO，只读 `presenter.ts` 已算好的字段，不重新判定），配 `SummaryView.tsx`。

**两轮机器人复审修掉的真问题**（都是「摘要跟组件详情自相矛盾」这一类）：
1. **「其余那些」一律说成「还在建」** —— 会把已经接完线、只差通电的组件说矮一档。改成按剩余组件的真实 bucket 生成措辞，两种状态混在一起时两种都说到；「老系统在跑」那句提示里的「新体系余项」同样处理。
2. **卡点归错人** —— 原来只要一条线上有任何组件在等拍板，整条线就写「卡住等你决定」，等于把代码/数据/上游依赖的技术卡点栽给 PM。改成只认**被卡住的那个组件本人**是否在 `decisionsNow` 里；「这条线另有事等你拍板」继续由 `needsYourCall` 字段 + UI 独立徽章表达。

**质量闸**：C 级（展示层，不碰判定逻辑与数据）。`summary.test.ts` 26 条 + `ProductMapClient.test.tsx` 17 条全绿 · `npm run build` 通过 · Codex 复审三轮，前两轮全修、第三轮判为改进单开跟进。

**follow-up（不阻塞上线）**：[#1085](https://github.com/bigbigraydeng-maker/magic-engine/issues/1085) —— 摘要里的「下一步」要带上组件名和具体卡点，现在只有成熟度标签，看的人不知道该推哪个组件。
