...
### 依赖已经解除，逐条 `gh issue view` + `gh pr list --search --state all` 核实过（2026-09-15）

> ⚠️ 上表最后三行（Verifier/agent core/F3）都已合并，但**还没能让 CTS 私信客服真正跑起来**——
> 前两行（Verifier + agent core）只是"判断该不该说 / AI 能查什么"这两层静态逻辑，F3 虽然已经
> 是真正的 Inngest 编排/写入端点，但它中继的是"人工已经批准"这个信号，**没有 F2（生成草稿+
> 真正调用发送）在中间跑，F3 唤醒不了任何东西**——下面这批 Inngest 编排/UI/dry-run 才是让它
> 真正上线还要做的事。已逐条核实，不是抄旧文档。
...
> ⚠️ 核实方法：`gh issue view <号>` 确认状态；`gh pr list --search "<号>" --state all` 逐个查有
> 没有已开/已合并/已关闭的 PR——**必须带 `--state all`**，`gh pr list` 不加这个参数默认只列
> `open`，会把已经合并的 PR 漏掉、把已完成的 issue 误判成"没人在动"（Codex 复审 2026-09-15
> 实测指出）。表格原本 9 个 Inngest/UI/dry-run 条目，#1586（F3）已完成，划掉保留在表里做记录
> （详情见上表），不再算"待认领"；剩 8 个里 #1584 命中
> 一个 open PR（#1736，当场在做，进这条线前先去看进度，不要重开）；其余 7 个（#1585/#1587-
> #1592）搜索无匹配 PR，状态是真的"没人在动"，不是"没查"。**这份核实是这次改动时的快照，
> 会过期**——下一个进这条线的窗口领活前仍要自己重跑一遍上面两条命令，不能直接信这张表。
