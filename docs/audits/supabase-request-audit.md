...
|---|---|---|---|---:|---|
| **P0-1** | 4 个 cron 24h 内 480 次运行 **零产出** | 降频：`*/10` → `0 * * * *`（每小时），`*/15` → `0,30 * * * *`；或改事件驱动 | `render.yaml`（`vision-analyzer` L111、`poll-visual-jobs` L126、`zhangqian-sweeper` L182、`blog-stuck-generating-sweeper` L235） | **~1,700–2,600 次/天（7–11%）** | 低。这 4 个都是"兜底扫描"，降频只增加最坏情况延迟 |
...
|---|---:|---:|---|
| P0-1（4 个空跑 cron 降频） | ~1,700–2,600/天 | **7–11%** | 480 次空跑 × (2 次记账 + 约 1.5 次探询)；降频至 1/6~1/2 |
| P0-2（记账合并为 1 次） | ~800–1,200/天 | **3–5%** | 808 次运行 × 1 次；P0-1 生效后基数变小 |
| P1-3 + P1-4（前端轮询修正） | ~100–500/天 | **0.5–2%** | 取决于管理页打开时长 |
| **合计** | **~2,600–4,300/天** | **约 11–17%** | |
