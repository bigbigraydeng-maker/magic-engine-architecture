...
| 路径 | 代码 | 真实产出 |
|---|---|---|
| `boost_post_api` 给帖子投流 | `meta-ads/boost-post/route.ts` | `ad_creative_links` **0 行** ⚠️ 只能读作「没有成功记下任何归因」，**不能读作「从没跑过」**（理由见下方注）|
...
> ⚠️ **空表不等于没跑过**（Codex 复审第十四轮 P2，核实成立；上一版把 `ad_creative_links` 0 行直接读成"未用过"）。`creative-link.ts:227-259` 的 `persistLink` **整段包在 try 里、把所有错误都吞掉**，而且这是**故意的**，注释写得很清楚：
>
> > 这一步失败只意味着「归因少了一条」，而让它抛出去会被上层当成建广告失败，进而重试建广告 —— 花钱建出重复广告，比丢一条归因严重得多。
>
...
→ **Oztop 六成多的钱压在 1 条创意上（单条约 61.9% = 63.4% × 97.6%，别把 campaign 合计当单条）；14 个角度的测试只分到 12%。**
...
**三家共同的形状（不依赖跨币种加总）：钱压在 1–2 条广告上，多角度测试只分到零头。**

> ⚠️ **这里数的是"广告条数"，不是"创意变体数"**（Codex 复审第十四轮 P2，核实成立；前几版都写成"创意数"）。`ad_daily_insights` 的 `level='ad'` 行只有 `entity_id` / `entity_name` + 成效指标，**没有 `creative_id`，也没有 `asset_feed_spec`**。所以：
>
...
- **Oztop 14 条 hook 的目标选对了。** 它是攒池实验（`OZ-THRU-S1-hooktest`，打法 `thruplay_pool_build`），要筛的是"哪个 hook 能最便宜地让人看完"，`THRUPLAY` 正是该用的目标，results=0 是设计如此。**但"目标选对"不等于"实验做对"** —— 见下面第三条，它还缺每臂最低探索量（第二十六轮补正，别把这一条读成"只要补指标就行"）。
...
6. **模板不会自行新增事实**：`listing-draft-builder` 没有自由文本入口，买家读到的每一句都是由 `ListingFacts` / `AgentFacts` 的字段拼出来的，**AI 不会凭空多写一个价格或一句战绩**；禁用词命中直接拒绝出稿（`BannedPhraseError`，不是标红让人审）。
  ...
  也就是说：付费调用方只要附一个任意 URL，就能让未证实的内容进入待批准广告，并带着"官网可溯"的标记。**准确的说法是"模板不会自行新增事实"，不是"结构上编不出假话"。** 想靠它安全地批量扩量（§7 ③），前置是补一道**真实的来源校验** —— 注意**不是**"按 `sourceUrl` 抓页核对"（那个 URL 也来自调用方，指向自己控制的页面即可绕过，还引入 SSRF），而是**把信任根换成客户已登记的域名**，或只接受 ME 已核实的数据记录；真要抓页必须走 `safe-fetch.ts` 的 `safeFetchText`。详见执行顺序第 4 步。
...
| 2 | **钱全压在单条广告上** | CTS 80.7% 的花费在 2 条广告上；Oztop 单条约 61.9%（其所在 campaign 合计 63.4%）（各自单一币种口径，见 §3.3）|
| 3 | **视频/攒池类实验读不出结论 —— 两个原因叠在一起，不是一个** | ① **指标没采**：`ad_daily_insights` 无完播次数 / 单次完播成本 / `video_p100` / 池子增长；14 条 hook 的 $432.50 在 ME 里只读得出「0 结果」，而那个数按 `ads-expected-metric.ts:108-123` 本就不该用来判好坏（`P21.K.8` 已登记未做）。② **探索量不足**：同组分配下一条吃掉 64.9%、3 条不足 $1，**没上过场的角度补再多指标也判不出输赢**（`AD-EXPL-1`）。⚠️ 只做 ① 仍然白测 |
...
| 11 | 🔴 **已上线的止损按钮能动别家客户的广告** | `meta-ads/execute/route.ts:71+` 的 `campaign_id` 取自请求体，只校验 URL 里的客户，**从不与该客户的 `meta_ad_account_id` 对账**；混账户下有 CTS 权限即可暂停 / 改预算 Oztop 的在投广告。**同一个模子的端点共有五个（第四十九轮全仓重扫确认，其中三个已上线）** —— `ad-health/stop-loss/route.ts:94-115` 是独立的第二个止损端点，`winner-reel-sync/engine.ts` 更是**连请求体都不经过**、直接信一张没人校验过的配置表。strategy doc §2.4 记的 R5 写越权，**至今未修**（登记为 `AD-SEC-1`）|
...
| 14 | 🔴 **production package 的「广告」那一栏从来就是空的 —— 根因是 migration 在仓库里、生产从没跑过** | `meta_ads_snapshots` **没有 `production_package_id` 这一列**（实查 `information_schema`）。⚠️ **不是"这个功能没做"**：仓库里 `20260522000001_p13d_production_package_links.sql` 同时给 `meta_ads_snapshots` 和 `project_reviews` 加了列，但生产账本里只有 `p13d_project_reviews_pkg_link` 那半边。实测 `project_reviews` ✅ 有、`competitor_snapshots` ✅ 有（走 `p13e_pre_competitor_snapshots`）、**只有 `meta_ads_snapshots` ❌ 没有** —— 口碑和竞品维度是好的，**断的只有广告这一维**。写侧 `sync/route.ts:119-127` 是个不 await 的 `.update({ production_package_id })`，必然报错、只进 `console.error`，接口照样返回 200；读侧 `production/[packageId]/route.ts:116-120` 用同一个不存在的列 `.eq()` 过滤，错误被 `?? []` 吞掉 —— **P13.D 这条链路自上线起一次都没通过**。⚠️ 而且 `production_package_id` **直接取自请求体、从不核对 `production_packages.client_id`**（鉴权只看 URL 里的客户），读侧查 snapshot 时也不带 `client_id` —— **补列的那一刻就变成"能往别家的交付物里塞自己的广告数据"**（读侧对 package 本身有隔离，所以是注入方向，不是读取方向）。跟 `AD-SEC-1` 同源：信请求体里的外部主键（登记为 `AD-PKG-1`）|
...
- **(ii) 先做 ①b**：让**相对安全**的那条路径也能记账（要 migration，见下）。它天然带 `lead_form` 打法（`OUTCOME_LEADS` / `LEAD_GENERATION`），**是唯一能端到端验完整条链路的路径**。

  ⚠️ **但"安全"要打个折 —— 批准那一刻不再回读**（Codex 复审第十四轮 P1，核实成立）：`createDraftForApproval` 建完回读并过闸一次，之后 `approveDraft` **只查 `flywheel_actions.payload.status === 'awaiting_approval'`**，确认后直接 `activatePublished`，**没有再跑一次 `fetchAdSetReadback` / `fetchAdCreativesReadback` / `checkLaunch`**。

  草案从建出来到有人点头，中间隔着几小时甚至几天，而它就躺在一个**多客户共用的广告账户**里。这期间任何人（或 Meta 自己）改了地区、受众、文案，**批准人看到的还是建的那一刻的快照，钱却是按改后的配置花的**。

  这一条格外值得记，因为它**违背的正是这个模块自己的立论** —— `launch-readback.ts` 开篇写着「创建接口的回显不含 Meta 自己补上的东西……**只有回读能看见**」。既然如此，激活前更该再看一眼。

  **修法很轻**：`approveDraft` 在 `activatePublished` 之前重跑一次回读 + `checkLaunch`，有 blocker 就拒绝激活并把新发现写回 payload。不需要 migration，也不需要 PM 点头。**建议并进 ①b 一起做。**
...
⚠️⚠️ **但这样得到的是"平台优化"，不是"可比较的角度实验" —— 两者别混为一谈**（Codex 复审第二十二轮 P2，核实成立；这是对本建议本身最要害的一条）。

**证据就在本文里**：Oztop 那 14 条 hook **正是这个结构**（同一个 ad set，让 Meta 分配），结果是 `OZ-S1-A-spill` 一条吃掉 **64.9%**，另外 3 条拿到不足 $1。**大多数角度根本没被探索过。**

**而本仓自己早就写明了原因** —— `ad-level-breakdown.ts:89-91`：

> Meta 在同一广告系列内会**先预测谁会转化再分配展示** —— 拿到 77 次展示的广告出 0 个结果是正常的，不是「输了」。把这种广告当成对照组会得出**反向结论**。

同一个文件还定了 `MIN_RESULTS_FOR_COMPARISON = 3`，并声明"本模块**从不宣称谁赢了**"。也就是说：**ME 现有的比较工具，本来就拒绝对这种数据下赢家结论。**

所以必须二选一，别指望一个动作同时拿到两样：

| 你要什么 | 怎么做 | 代价 |
|---|---|---|
| **投放效率**（Andromeda 想要的） | 就按上面说的，全塞一个 ad set 让 Meta 分配 | 学不到"哪个角度好"，只学到"Meta 挑了哪个" |
| **可比较的角度实验** | 需要**每臂最低探索量**：设每臂最低展示/花费下限、分批放量、或直接用 Meta 原生 A/B test（`ads_experiment_abtest_*`，它会做公平分流） | 更贵，而且违背"别人为拆分"的直觉 —— 但这是拿到可比结论的唯一办法 |

**本审计的立场**：③ 的主要价值是**让创意数量上去**（从 1–2 条到 5–8 条），这一条无论走哪种都成立。但**如果目标是"找出哪句话最打动人"，就必须显式设探索下限，否则重演 Oztop 那次**。这一项**没有登记过**，已补为 `AD-EXPL-1`。
...
4. **补事实来源校验** —— ⚠️ **这一步是第十三轮才前移到这里的**（Codex P1，核实成立）。上一版把它挂在第 5 步（批量角度）之前，理由是"批量扩会放大这个洞"—— **但第 4 步已经在花真钱了**。`assertFacts` 只查 `sourceUrl` 非空、从不抓页面核对，而 `traceClaims` 照盖"官网可溯"章（§4 第 6 条）。所以**只要调用方给错房价/地址/战绩，第一条付费广告就会带着未核实内容过审、投出去** —— 这不是"量大了才危险"，是第一条就危险。
   ...
   **同一步一起做掉另外两条,都是第一条就会踩的**：
   ...
   - 🔴 **`AD-SPEC-1` 房源广告没声明「住房」特殊类别**（第三十八轮补入首投前置，**优先级等同 `AD-GEO-0`**）：`ad-publisher.ts:176` 写死 `special_ad_categories: JSON.stringify([])`，而首投要走的 `draft-listing` 正是**房源**（Roman / 30 Kiteroa 是住宅），客户服务协议（`docs/clients/30-kiteroa-rothesay-bay/service-agreement/2026-07-13-30-kiteroa-lead-gen-test-v1.html:380-383`）也明确要求遵守住房广告与反歧视政策。闸门也看不见这个字段（`launch-readback.ts` / `readback.ts` / `meta-readback-adapter.ts` 里 `special` 零命中）—— **建出来不声明、回读也不查，轻则拒登，重则违反投放限制**。
     ⚠️ **而且它跟 `AD-GEO-0` 是同一批要一起设计的**：住房类别下 Meta 会**限制定向能力并对地理半径设下限**（具体口径以 Meta 现行政策为准，**动手前必须查一次官方文档，别照本文的记忆值写**）。也就是说 `AD-GEO-0` 那个"改成只发城市 + 10km 半径"的修法，**在房源广告上可能直接不成立**。两条必须一起定，否则 geo 那条会按一个用不了的方案做出来。
   - 🔴 **`AD-FACT-1` 的「逐句来源要送到批准人面前」那一半**（第五十轮补入首投前置）：`draft-listing/route.ts:306-312` 的 `traceClaims` **只写进 HTTP 响应**，`DraftRecord` 里没有 `claims` 字段，审批页也只显示 `buyerWillSee`。**所以就算来源校验全做对了，批准人也看不到** —— 到他面前时那份逐字段来源已经不存在。校验做得再对，结论没传到决策点等于没做。修：`claims` 一起落进草案记录，并在「开始投放」按钮前逐句显示（官网可溯 / brief 可溯 / 未证实）。
   - 🔴 **`AD-QUEUE-1` 审批页会把早的待批草案挤掉**（第四十三轮补入首投前置）：`ad-approval/page.tsx:34-44` 查询**不带状态条件**、只取最近 50 条，然后才在内存里筛 `awaiting_approval`，页面还明写「不做分页」。攒够 50 条别的状态，**更早那条等你点头的就从唯一入口消失了**，而底部文案只说"还有更早的记录没显示"，看的人会当成历史。修：状态条件下推到数据库查。
   - 🔴 **`AD-LOG-1` 账本写失败时，广告已经建在 Meta 上而接口照报成功**（第四十五轮补入首投前置 —— 这条早就登记了，但一直没进"投第一条之前"这张清单）：`draft-and-gate.ts:70-82` 的 `record()` **不读 Supabase 的 `error`**，插入失败就返回 `null`；而 `:158-165` 拿到 `null` 照样返回 `status: 'awaiting_approval'`。此时 campaign / ad set / creative / ads **全都已经建好了**。
     ⚠️ **这比 `AD-ORPH-1` 更糟，不是同一回事**：blocked / rejected 至少还有一条账本行记着 `campaignId` / `adSetId` / `adIds`，后面的人能顺着找；而这种情况**连行都没有** —— 那套暂停实体在 Meta 里彻底失联，既不能批也不能否，**没有任何 ME 里的东西知道它存在**。调用方看到"成功"于是重试，**再建一整套**。修：账本写失败即回滚已建实体，回滚不掉就下发完整人工任务。
   - 🔴 **`AD-ORPH-1` 拦下 / 否决的草案在 Meta 那边没人收**（第四十三轮补入）：建都建出来了（暂停着），blocked 和 rejected 两条路**都只改账本、不删实体、也不下发人工任务**，而 `publishDraftPaused` 的逆序回滚只在"建到一半失败"时才跑。一堆永远不会再被批准的暂停广告堆在后台，**跟正常草案长得一样，后台的人一手滑就能开起来花钱**。
   - 🔴 **`AD-GATE-1` 的"回读要看得见钱、看得见文案"那两条**（第三十九轮补入首投前置）：① `readback.ts:27-29` 的 `ADSET_FIELDS` **不含 `daily_budget` / `lifetime_budget` / 排期**，所以"激活前重跑回读"**看不见预算被人改过** —— 而这恰恰是 `AD-GATE-1` 立项时说要修的那个场景；② `launch-readback.ts:294-303` 的 `creative_without_buyer_text` 是 `warn` 不是 `blocker`，照样放行，**而且创意数组为空时连这条 warn 都不产生** —— "一条广告都没回读到"和"全都干净"在闸门眼里一样。两条都得在首投前变成 blocker。
...
2. **广告"说什么"这件事，我们还在用老办法。** 钱最多的三条广告，**在系统里只看得到 4 条广告**（⚠️ 一条广告内部可能还装着好几套文案，我们现在查不出来 —— 所以准确说法是"钱压在极少数广告上"，"片子种类少"要再去 Facebook 后台核一次才能确认）；而我们真正试过十几个不同说法的那两次，一次花了 $432、一次花了 $366，**两次都没能得出结论**。

  Oztop 那次（$432）**选的广告类型没错**（它本来就是"攒人"用的，不是"收询盘"用的），但有**两个毛病叠在一起**：
  - 我们的系统**没把该看的数收进来**（多少人把片子看完、看完一次多少钱）；
  - 就算收进来了也不够 —— 那十几个说法**根本没被公平地试过**：一条吃掉了六成半的钱，还有三条连一块钱都没花到。**没上过场的说法，收再多指标也判不出输赢。**

  Roman 那次（$366）是钱太少，十几条广告分下去每条只有二十几块，谁赢基本靠运气。

  ⚠️ **所以别只补指标就以为解决了** —— 补指标（`P21.K.8`）和保证每个说法都被试够（`AD-EXPL-1`，要额外花钱、要你拍板）**是两件事，缺一件都还是白测**。
...
3. **最关键的一环还没通电：我们至今说不清哪一条片子带来了哪一个客户。** 39 个已经追到"哪条广告"的客户里，**没有一个**能追到"哪条片子"。记这件事的表已经建好，但**记录这件事的几段路只各修了一段、还没接到一起**。
  ...
  - 另一条路（ME 从素材直接起草一条新广告）**把关基本都有**（先建成暂停、检查一遍、你点头才开），而且它是**收联系方式**的广告，能真的产生询盘 —— **只有这条路能把"哪条片子带来哪个客户"整条验通**。它现在的问题是**记不下来**是哪条片子，要记得下来得改一次数据库结构。

    ⚠️ 还有个小毛病要顺手修：**你点"开"的那一刻，系统不会再检查一遍** —— 检查是在广告刚建出来时做的，可能是几天前。这中间如果有人在共用的广告账户里改了投放地区或文案，你看到的还是旧的，钱却按新的花。修起来很轻，会跟上面那件事一起做。
...
>
> 所以我们能做到的是：**一次投十几条真正不同说法的广告，让平台自己去找哪种人吃哪一套，然后照着跑得最好的那句话再生成下一批。** 这个能力代理公司做不了（他们出不起十几条片子的人工），普通自助工具也做不了（他们不认识客户的真实产品）。
>
> ⚠️ **但有一句话我原来写得太满，得收回来**：上一版说"两周内告诉客户你的生意真正打动人的是哪句话"。**做不到 —— 至少不是把十几条一起丢给平台就能做到的。** Oztop 那次就是这么投的：十几个说法里，**一条吃掉了六成半的钱，还有三条连一块钱都没花到**，绝大多数说法根本没被试过。平台会很快押注在它自己看好的那条上，这对省钱是好事，但**它给不了"哪句话更打动人"的答案**，因为大部分句子压根没上过场。
>
> 想真拿到那个答案，得**额外花钱保证每句话都被试够**（给每条设最低曝光/花费，或用 Facebook 自带的对照测试功能）。这是一笔要单独决定的预算 —— **要不要花，是你的决定，不是技术细节。** 现在这一项已经登记（`AD-EXPL-1`），没做之前，我们能诚实承诺的是"**多试几种说法、让平台挑出跑得最好的**"，不是"**告诉你哪句话最打动人**"。
>
...
4. **🆕 广告花费表都没有币种列，跨客户金额不可加总** —— 本审计发现的新缺口，**ROADMAP 里没有登记过**，而且**是两张表**（第十八轮更正：上一版只点了 `ad_daily_insights`，把影响面挂错了表）：
   ...
   ✅ **已登记为 `AD-CUR-1`**（`docs/ROADMAP.md` §广告引擎中心）。第十六轮更正 —— 上一版写的是"建议登记，不在本审计范围内"，那等于把发现留在文档里等人捡，正是 CLAUDE.md 铁律 3 下半禁止的「发现死在日志里」，也违反 §9「未完成任务回写 ROADMAP」。本次审计的全部新发现与未完成动作已一并登记（**22 条**）：`AD-CUR-1/2` · `AD-PLAY-1` · `AD-GATE-1` · `AD-SEC-1/2` · `AD-FACT-1` · **`AD-FORM-1`**（选表单不看语言，首条真钱广告的前置）· `AD-OBS-1/2` · `AD-LOG-1` · `AD-ADV-1` · `AD-DRAFT-1` · `AD-LINK-1` · **`AD-EXPL-1`**（每臂最低探索量，决定「找出赢家」这个卖点成不成立）· 🟠 **`AD-ISO-1`**（生产同步把整账户写成单个客户，已上线在跑；105 行里 101 行实测干净、2 行丢了明细无从核对，是隐患不是已确认的损害）· 🔴 **`AD-PKG-1`**（交付物的广告栏从上线起就是空的；根因是 `20260522000001` 这份 migration 在仓库里、生产只跑了 `project_reviews` 那半边，写读两侧都指向不存在的列且都不报错；补列前须先补包归属校验，否则一补列就变成跨客户注入）· 🔴 **`AD-SPEC-1`**（房源广告没声明「住房」特殊类别，建不声明、回读不查，须与 `AD-GEO-0` 同批设计）· 🔴 **`AD-GEO-0`**（`targetingFor` 国家+城市并集，ME 起草的广告天生投整个国家 —— 首发前必修）· **`AD-GEO-1`**（geo 闸门只到国家级，抓不到它自己引用的那次事故）· `AD-EVID-1` · `AD-FRAG-1`。
