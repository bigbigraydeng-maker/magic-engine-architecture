...
2. **广告"说什么"这件事，我们还在用老办法。** 钱最多的三条广告，**在系统里只看得到 4 条广告**（⚠️ 一条广告内部可能还装着好几套文案，我们现在查不出来 —— 所以准确说法是"钱压在极少数广告上"，"片子种类少"要再去 Facebook 后台核一次才能确认）；而我们真正试过十几个不同说法的那两次，一次花了 $432、一次花了 $366 —— **两次都没能得出结论，但原因不一样**：Oztop 那次（$432）**方法是对的**，是我们的系统没把该看的数（多少人把片子看完、看完一次多少钱）收进来，所以现在读不出谁赢；Roman 那次（$366）是钱太少，十几条广告分下去每条只有二十几块，谁赢基本靠运气。
