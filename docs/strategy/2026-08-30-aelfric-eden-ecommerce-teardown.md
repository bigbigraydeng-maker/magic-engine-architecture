...
**为什么这招狠**：
1. 创作者发内容 → 落到"以他名字命名的页" → 归因**有了可汇总的落点**（一个 URL 一个人）。
   ⚠️ **但 URL 本身不等于干净归因**：这些页面同样会被自然搜索和 AI 引用（见第 3 点），社媒墙（§3.2）也直接链到同一个 collection，所以按 URL 汇总会把自然搜索、AI 推荐和站内点击的转化一起算进创作者账上。真要归因，只能认带**创作者专属 UTM、专属折扣码或其他可验证触点**的访问；否则后续实验会系统性高估这个机制的效果
...
Loox 的评价是客户端渲染的，服务端交给 Google 的这份结构化数据是一个**无效评分**。

**测量缺口（结论未成立）**：本次只抓了原始 HTML。Googlebot 会执行 JavaScript，Loox 完全可能在渲染后的 DOM 里再注入一份有效的评价结构化数据，因此服务端出现 `ratingValue: 0` **不能单独证明最终富摘要失效**。要定性为故障，必须先检查渲染后 DOM 或跑一次 Google Rich Results Test；在补这一步之前，下面的范围订正与 §7.4 的自动检测候选都只能当作**待验证的检测方向**，不是已确认缺陷。
...
- **UGC 图片评价（Loox 一类）· 待验证假设** —— AE 用的是 Loox，但 §9 已声明未查营收与转化率，本次也没有测过 Jing's Pick 客群的评价偏好，**"华人客群对真人实拍更敏感"目前是假设不是结论**。验证方式：先看现有商品页的评价曝光与加购率基线，再定是否上图片评价 App（含月费成本）
- **BNPL · 待验证假设** —— AE 用的是 Klarna + Sezzle。本报告没有 Jing's Pick 的客单价、现有支付组合、BNPL 手续费率和转化数据，**"Afterpay/Laybuy 对客单价有实际拉动"不能当已验证结论下发**。验证方式：先拉现有客单价分布与弃单率基线，算清 BNPL 手续费对毛利的侵蚀，再决定是否接
