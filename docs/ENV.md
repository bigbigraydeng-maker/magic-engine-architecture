...
| `CLOUDFLARE_MGMT_TOKEN` | 客户站点 DNS / Pages 管理；Customer Site Cache Refresh Inngest 函数用它 purge 每个客户站的 CF 边缘缓存（zone id 从 `client_site_platforms` 表逐客户取，不落 env） | Render-web | ✅ |
